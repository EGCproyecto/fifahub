...
class IEDriver(Driver):
    ...
    def __init__(self, name, driver_version, url, latest_release_url, ie_release_tag, http_client, os_system_manager):
        super(IEDriver, self).__init__(name, driver_version, url, latest_release_url, http_client, os_system_manager)
    ...
    def get_latest_release_version(self) -> str:
        ...
        releases = resp.json()
        release = next(
            release for release in releases for asset in release["assets"] if asset["name"].startswith(self.get_name())
        )
    ...
    def get_driver_download_url(self, os_type):
        ...
        resp = self._http_client.get(url=self.tagged_release_url(driver_version_to_download), headers=self.auth_header)
