...
from antlr4.atn.ATNState import ATNState, DecisionState, RuleStopState
...
class ParserATNSimulator(ATNSimulator):
    ...
    def closure(
        ...
        self.closureCheckingStopState(
            config, configs, closureBusy, collectPredicates, fullCtx, initialDepth, treatEofAsEpsilon
        )
            ...
            if not config.context.isEmpty():
                for i in range(0, len(config.context)):
                    ...
                    if state is PredictionContext.EMPTY_RETURN_STATE:
                        if fullCtx:
                            configs.add(
                                ATNConfig(state=config.state, context=PredictionContext.EMPTY, config=config),
                                self.mergeCache,
                            )
                            continue
                        else:
                    ...
                    c = ATNConfig(
                        state=returnState, alt=config.alt, context=newContext, semantic=config.semanticContext
                    )
    ...
    epsilonTargetMethods[Transition.RULE] = (
        lambda sim, config, t, collectPredicates, inContext, fullCtx, treatEofAsEpsilon: sim.ruleTransition(config, t)
    )
    ...
    epsilonTargetMethods[Transition.ACTION] = (
        lambda sim, config, t, collectPredicates, inContext, fullCtx, treatEofAsEpsilon: sim.actionTransition(config, t)
    )
