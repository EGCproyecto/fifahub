...
class XPathRuleAnywhereElement(XPathElement):
    ...
    def evaluate(self, t: ParseTree):
        ...
        return filter(
            lambda c: isinstance(c, ParserRuleContext) and (self.invert ^ (c.getRuleIndex() == self.ruleIndex)),
            Trees.descendants(t),
        )
...
class XPathTokenAnywhereElement(XPathElement):
    ...
    def evaluate(self, t: ParseTree):
        ...
        return filter(
            lambda c: isinstance(c, TerminalNode) and (self.invert ^ (c.symbol.type == self.tokenType)),
            Trees.descendants(t),
        )
