...
class PropertyObserver:
        ...
        for class_, generators in self.generator_registry.items():
            for callback in generators:
                ...
                for path in full_paths:
                    self.callback_map[class_].append(Callback(func=callback, backref=None, fullpath=full_paths))
                    ...
                    for index in range(len(path)):
                        ...
                        if isinstance(prop, sa.orm.RelationshipProperty):
                            ...
                            self.callback_map[prop_class].append(
                                Callback(func=callback, backref=~(path[:i]), fullpath=full_paths)
                            )
    ...
    def get_callback_args(self, root_obj, callback):
        ...
        objects = [getdotattr(root_obj, path, lambda obj: obj not in session.deleted) for path in callback.fullpath]
            ...
            for root_obj, func, objects in args:
                ...
                for i, object_ in enumerate(objects):
                    if is_sequence(object_):
                        callback_args[root_obj][func][i] = callback_args[root_obj][func].get(i, set()) | set(object_)
                    else:
