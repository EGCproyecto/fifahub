...
from .relationships import chained_join, path_to_relationships, select_correlated_expression
...
class AggregatedValue:
    ...
    def aggregate_query(self):
        query = select_correlated_expression(self.class_, self.expr, self.path, self.relationships[0].mapper.class_)
...
class AggregationManager:
    ...
    def register_listeners(self):
        sa.event.listen(sa.orm.Mapper, "after_configured", self.update_generator_registry)
        sa.event.listen(sa.orm.session.Session, "after_flush", self.construct_aggregate_queries)

    def update_generator_registry(self):
        for class_, attrs in aggregated_attrs.items():
            for expr, path, column in attrs:
                value = AggregatedValue(class_=class_, attr=column, path=path, expr=expr(class_))
