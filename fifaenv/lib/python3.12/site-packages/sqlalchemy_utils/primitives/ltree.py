...
class Ltree:
    ...
    def __init__(self, path_or_ltree):
        if isinstance(path_or_ltree, Ltree):
            ...
            raise TypeError(
                "Ltree() argument must be a string or an Ltree, not '{}'".format(type(path_or_ltree).__name__)
            )
    ...
    def __getitem__(self, key):
        ...
        raise TypeError("Ltree indices must be integers, not {}".format(key.__class__.__name__))
    ...
    def lca(self, *others):
        ...
        for index, element in enumerate(parts):
            if any(
                other[index] != element or len(other) <= index + 1 or len(parts) == index + 1 for other in other_parts
            ):
                if index == 0:
                    return None
