...
class WeekDays:
    def __init__(self, bit_string_or_week_days):
        if isinstance(bit_string_or_week_days, str):
            ...
            if len(bit_string_or_week_days) != WeekDay.NUM_WEEK_DAYS:
                raise ValueError("Bit string must be {} characters long.".format(WeekDay.NUM_WEEK_DAYS))
    ...
    def as_bit_string(self):
        return "".join("1" if WeekDay(index) in self._days else "0" for index in range(WeekDay.NUM_WEEK_DAYS))
