...
def inverse_join(selectable, left_alias, right_alias, relationship):
    if relationship.property.secondary is not None:
        ...
        return selectable.join(
            secondary_alias,
            adapt_expr(relationship.property.secondaryjoin, sa.inspect(left_alias).selectable, secondary_alias),
        ).join(
            right_alias,
            adapt_expr(relationship.property.primaryjoin, sa.inspect(right_alias).selectable, secondary_alias),
        )
    else:
...
def chained_inverse_join(relationships, leaf_model):
    ...
    for index, relationship in enumerate(relationships[1:]):
        ...
        selectable = inverse_join(selectable, aliases[index], aliases[index + 1], relationships[index])
    ...
    if relationships[-1].property.secondary is not None:
        ...
        selectable = selectable.join(
            secondary_alias,
            adapt_expr(relationships[-1].property.secondaryjoin, secondary_alias, sa.inspect(aliases[-1]).selectable),
        )
        aliases.append(secondary_alias)
    return selectable, aliases


def select_correlated_expression(root_model, expr, path, leaf_model, from_obj=None, order_by=None, correlate=True):
    ...
    if order_by:
        query = query.order_by(*[adapt_expr(o, *(sa.inspect(alias).selectable for alias in aliases)) for o in order_by])
