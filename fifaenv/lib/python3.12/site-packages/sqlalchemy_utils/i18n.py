...
class TranslationHybrid:
    def __init__(self, current_locale, default_locale, default_value=None):
        if babel is None:
            raise ImproperlyConfigured("You need to install babel in order to use TranslationHybrid.")
    ...
    def __call__(self, attr):
        return hybrid_property(
            fget=self.getter_factory(attr), fset=self.setter_factory(attr), expr=self.expr_factory(attr)
        )
