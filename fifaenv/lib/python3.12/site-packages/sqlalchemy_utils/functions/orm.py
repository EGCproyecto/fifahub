...
def get_class_by_table(base, table, data=None):
    ...
    """
    found_classes = {c for c in _get_class_registry(base).values() if hasattr(c, "__table__") and c.__table__ is table}
...
def cast_if(expression, type_):
    ...
    return sa.cast(expression, type_) if not isinstance(expr_type, check_type) else expression
    ...
    try:
        ...
        for key, c in mapper.columns.items():
            if c.name == column.name and c.table is column.table:
                return key
    raise sa.orm.exc.UnmappedColumnError("No column %s is configured on mapper %s..." % (column, mapper))
...
def identity(obj_or_class):
    ...
    :param obj: SQLAlchemy declarative model object
    """
    return tuple(getattr(obj_or_class, column_key) for column_key in get_primary_keys(obj_or_class).keys())
