...
def merge_references(from_, to, foreign_keys=None):
    ...
    for fk in foreign_keys:
        ...
        criteria = (getattr(fk.constraint.table.c, key.key) == value for key, value in old_values.items())
        query = (
...
def dependent_objects(obj, foreign_keys=None):
    ...
    for table, keys in group_foreign_keys(foreign_keys):
        ...
        for class_ in classes.values():
            ...
            if table in mapper.tables and not (parent_mapper and table in parent_mapper.tables):
                query = session.query(class_).filter(sa.or_(*_get_criteria(keys, class_, obj)))
    ...
    for key in keys:
        ...
        for index, column in enumerate(key.constraint.columns):
            ...
            subcriteria.append(
                ...
                == getattr(obj, sa.inspect(type(obj)).get_property_by_column(foreign_column).key)
            )
...
def non_indexed_foreign_keys(metadata, engine=None):
    ...
    if bind is None and engine is None:
        raise Exception("Either pass a metadata object with bind or " "pass engine as a second parameter")
