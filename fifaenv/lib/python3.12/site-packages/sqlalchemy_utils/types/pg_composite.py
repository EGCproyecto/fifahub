    ...
    class comparator_factory(UserDefinedType.Comparator):
        def __getattr__(self, key):
            try:
                ...
                raise KeyError("Type '{}' doesn't have an attribute: '{}'".format(self.name, key))

            return CompositeElement(self.expr, key, type_)

    def __init__(self, name, columns, quote=None, **kwargs):
        if psycopg2 is None:
            raise ImproperlyConfigured("'psycopg2' package is required in order to use CompositeType.")
    ...
    def bind_processor(self, dialect):
        def process(value):
            ...
            for i, column in enumerate(self.columns):
                current_value = value.get(column.name) if isinstance(value, dict) else value[i]
...
def register_psycopg2_composite(dbapi_connection, composite):
    psycopg2.extras.register_composite(composite.name, dbapi_connection, globally=True, factory=composite.caster)
    ...
    def adapt_composite(value):
        ...
        adapted = [
            adapt(
                ...
                else column.type.process_bind_param(getattr(value, column.name), dialect)
            )
        ...
        return AsIs("({})::{}".format(", ".join(values), dialect.identifier_preparer.quote(composite.name)))
...
def _visit_create_composite_type(create, compiler, **kw):
    ...
    fields = ", ".join(
        "{name} {type}".format(name=column.name, type=compiler.dialect.type_compiler.process(to_instance(column.type)))
        for column in type_.columns
    )

    return "CREATE TYPE {name} AS ({fields})".format(name=compiler.preparer.format_type(type_), fields=fields)
