...
from .model import BindMixin, DefaultMeta, DefaultMetaNoName, Model, NameMixin, _QueryProperty
...
def _get_2x_declarative_bases(
    ...
    return [b for b in model_class.__bases__ if issubclass(b, (sa_orm.DeclarativeBase, sa_orm.DeclarativeBaseNoMeta))]
...
class SQLAlchemy:
    ...
    def __init__(
        ...
        *,
        ...
        self.Model = self._make_declarative_base(model_class, disable_autonaming=disable_autonaming)
    ...
    def init_app(self, app: Flask) -> None:
        ...
        basic_uri: str | sa.engine.URL | None = app.config.setdefault("SQLALCHEMY_DATABASE_URI", None)
        ...
        config_binds: dict[str | None, str | sa.engine.URL | dict[str, t.Any]] = app.config.setdefault(
            "SQLALCHEMY_BINDS", {}
        )
    ...
    def _make_scoped_session(self, options: dict[str, t.Any]) -> sa_orm.scoped_session[Session]:
    ...
    def _make_session_factory(self, options: dict[str, t.Any]) -> sa_orm.sessionmaker[Session]:
    ...
    def _make_declarative_base(
        ...
        if len(declarative_bases) > 1:
            ...
            raise ValueError(
                "Only one declarative base can be passed to SQLAlchemy." " Got: {}".format(model_class.__bases__)
            )
            ...
            model = sa_orm.declarative_base(metadata=metadata, cls=model_class, name="Model", metaclass=metaclass)
        else:
    ...
    def _apply_driver_defaults(self, options: dict[str, t.Any], app: Flask) -> None:
        ...
        if url.drivername in {"sqlite", "sqlite+pysqlite"}:
            ...
            if "pool_class" not in options or options["pool_class"] is sa.pool.QueuePool:
                options.setdefault("pool_recycle", 7200)

            if "charset" not in url.query:
                options["url"] = url.update_query_dict({"charset": "utf8mb4"})

    def _make_engine(self, bind_key: str | None, options: dict[str, t.Any], app: Flask) -> sa.engine.Engine:
    ...
    def get_engine(self, bind_key: str | None = None, **kwargs: t.Any) -> sa.engine.Engine:
    ...
    def first_or_404(self, statement: sa.sql.Select[t.Any], *, description: str | None = None) -> t.Any:
    ...
    def one_or_404(self, statement: sa.sql.Select[t.Any], *, description: str | None = None) -> t.Any:
    ...
    def _relation(self, *args: t.Any, **kwargs: t.Any) -> sa_orm.RelationshipProperty[t.Any]:
        """A :func:`sqlalchemy.orm.relationship` that applies this extension's
        :attr:`Query` class for dynamic relationships and backrefs.
