...
class frozendict(FrozenDict):
    ...
    def __init__(self, *args: Any, **kwargs: Any):
        warnings.warn(
            f"{self.__class__.__name__} is deprecated. " "Use pydot.classes.FrozenDict instead.",
...
def quote_id_if_necessary(s: str, unquoted_keywords: Sequence[str] | None = None) -> str:
...
def graph_from_dot_file(path: str | bytes, encoding: str | None = None) -> list[Dot] | None:
...
def graph_from_edges(edge_list: Sequence[Any], node_prefix: str = "", directed: bool = False) -> Dot:
...
class Node(Common):
    ...
    def to_string(self, indent: Any = "", indent_level: int = 1) -> str:
        ...
        node = quote_id_if_necessary(self.obj_dict["name"], unquoted_keywords=("graph", "node", "edge"))

        # No point in having default nodes that don't set any attributes...
        if node in ("graph", "node", "edge") and len(self.obj_dict.get("attributes", {})) == 0:
            return ""
...
class Edge(Common):
    ...
    def parse_node_ref(self, node_ref: EdgeEndpoint) -> EdgeEndpoint:
        ...
        if node_port_idx > 0 and node_ref[0] == '"' and node_ref[node_port_idx - 1] == '"':
            return node_ref
    ...
    def to_string(self, indent: Any = "", indent_level: int = 1) -> str:
        ...
        if isinstance(src, FrozenDict):
            ...
            edge = [sgraph.to_string(indent=indent, indent_level=indent_level, inline=True)]
        else:
            edge = [str(src)]
        ...
        if isinstance(dst, FrozenDict):
            ...
            edge.append(sgraph.to_string(indent=indent, indent_level=indent_level, inline=True))
        else:
            edge.append(str(dst))
...
class Graph(Common):
        ...
        **attrs: Any,
        ...
        if obj_dict is None:
            ...
            if graph_type not in ["graph", "digraph"]:
                raise pydot.Error(f'Invalid type "{graph_type}". ' "Accepted graph types are: graph, digraph")
    ...
    def del_edge(self, src_or_list: Any, dst: Any = None, index: int | None = None) -> bool:
    ...
    def to_string(self, indent: Any = "", indent_level: int = 0, inline: bool = False) -> str:
        ...
        if edge_obj_dicts:
            edge_ep_set = set(itertools.chain.from_iterable(obj["points"] for obj in edge_obj_dicts))
        else:
            edge_ep_set = set()
        ...
        for idx, obj in obj_list:
            if obj["type"] == "node":
                ...
                sgraph_str = Subgraph(obj_dict=obj).to_string(indent=indent, indent_level=indent_level + 1)
...
class Dot(Graph):
    ...
    def create(
        ...
        assert process.returncode == 0, f'"{prog}" with args {arguments} ' f"returned code: {process.returncode}"
