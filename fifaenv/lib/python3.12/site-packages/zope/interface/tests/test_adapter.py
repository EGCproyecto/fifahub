...
class BaseAdapterRegistryTests(unittest.TestCase):
    ...
    def test_register(self):
        ...
        self.assertEqual(registered, [((IB0,), IR0, "", "A1")])  # required  # provided  # name  # value
        ...
        def build_adapters(L, MT):
            return L(
                [
                    ...
                    # 1
                    MT({IB0: MT({IR0: MT({"": "A1", "name2": "A2"}), IR1: MT({"": "A3", "name3": "A4"})})}),
                    # 3
                    MT(
                        {
                            IB0: MT(
    ...
    def test_register_with_same_value(self):
        ...
        registered = list(registry.allRegistrations())
        self.assertEqual(registered, [((Interface,), IR0, "", _value)])  # required  # provided  # name  # value
    ...
    def test_unregister_hit_clears_empty_subcomponents(self):
        ...
        MT = self._getMappingType()
        self.assertEqual(
            registry._adapters[2], MT({IB1: MT({IB2: MT({None: MT({"": one})}), IB3: MT({None: MT({"": another})})})})
        )
        PT = self._getProvidedType()
    ...
    def test_subscribe_multiple_allRegistrations(self):
        ...
        def build_subscribers(L, F, MT):
            return L(
                [
                    ...
                    MT({IR0: MT({"": F(["A1", "A2"])})}),
                    # 1
                    MT({IB0: MT({IR0: MT({"": F(["A1", "A2"])}), IR1: MT({"": F(["A3", "A4"])})})}),
                    # 3
                    MT(
                        {
                            IB0: MT(
        ...
        self.assertEqual(
            registry._subscribers,
            build_subscribers(L=self._getMutableListType(), F=self._getLeafSequenceType(), MT=self._getMappingType()),
        )
        ...
        self.assertEqual(
            registry._subscribers, build_subscribers(L=CustomSequence, F=CustomLeafSequence, MT=CustomMapping)
        )
