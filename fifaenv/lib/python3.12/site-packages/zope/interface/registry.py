...
from zope.interface.declarations import implementedBy, implementer, implementer_only, providedBy
...
class Components:
    ...
    def __reduce__(self):
        ...
        filtered_state = {k: v for k, v in reduction[2].items() if not k.startswith("_v_")}
    ...
    def registerUtility(self, component=None, provided=None, name="", info="", event=True, factory=None):
        if factory:
            if component:
        ...
        self._utility_registrations_cache.registerUtility(provided, name, component, info, factory)

        if event:
            notify(Registered(UtilityRegistration(self, provided, name, component, info, factory)))
    ...
    def unregisterUtility(self, component=None, provided=None, name="", factory=None):
        ...
        notify(Unregistered(UtilityRegistration(self, provided, name, component, *old[1:])))
    ...
    def registerAdapter(self, factory, required=None, provided=None, name="", info="", event=True):
        if provided is None:
        ...
        if event:
            notify(Registered(AdapterRegistration(self, required, provided, name, factory, info)))
    ...
    def registeredAdapters(self):
        for (required, provided, name), (component, info) in iter(self._adapter_registrations.items()):
    ...
    def registerSubscriptionAdapter(self, factory, required=None, provided=None, name="", info="", event=True):
        if name:
            raise TypeError("Named subscribers are not yet supported")
        if provided is None:
            provided = _getAdapterProvided(factory)
        required = _getAdapterRequired(factory, required)
        self._subscription_registrations.append((required, provided, name, factory, info))
    ...
    def unregisterHandler(self, factory=None, required=None, name=""):
        ...
        if factory is None:
            new = [(r, n, f, i) for (r, n, f, i) in self._handler_registrations if r != required]
        else:
            new = [(r, n, f, i) for (r, n, f, i) in self._handler_registrations if not (r == required and f == factory)]
...
def _getAdapterProvided(factory):
    ...
    if len(provided) == 1:
        return provided[0]
    raise TypeError(
        "The adapter factory doesn't implement a single interface " "and no provided interface was specified."
    )
...
class AdapterRegistration:
    ...
    def __init__(self, registry, required, provided, name, component, doc):
        (self.registry, self.required, self.provided, self.name, self.factory, self.info) = (
            registry,
            required,
            provided,
            name,
