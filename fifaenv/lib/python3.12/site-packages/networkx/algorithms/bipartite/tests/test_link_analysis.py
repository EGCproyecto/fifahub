...
class TestBipartiteLinkAnalysis:
    ...
    def setup_class(cls):
        ...
        cls.women_bipartite_set = {
            node for node, bipartite in cls.davis_southern_women_graph.nodes(data="bipartite") if bipartite == 0
        }
        cls.gnmk_random_graph = nx.bipartite.generators.gnmk_random_graph(5 * 10**2, 10**2, 5 * 10**2, seed=27)
        cls.gnmk_random_graph_top_nodes = {
            node for node, bipartite in cls.gnmk_random_graph.nodes(data="bipartite") if bipartite == 0
        }
    ...
    def test_collaborative_filtering_birank(self):
        ...
        u1_query = {
            product: rating for _, product, rating in item_recommendation_graph.edges(nbunch="u1", data="rating")
        }
        ...
        assert u1_birank_results_unweighted["p2"] == pytest.approx(u1_birank_results_unweighted["p3"], rel=2e-6)
    ...
    @pytest.mark.parametrize("damping_factor,value", itertools.product(["alpha", "beta"], [-0.1, 1.1]))
    ...
    def test_gnmk_convergence_birank(self, personalization, alpha, beta):
        top_personalization_dict = {node: personalization(node) for node in self.gnmk_random_graph_top_nodes}
        bipartite.birank(
