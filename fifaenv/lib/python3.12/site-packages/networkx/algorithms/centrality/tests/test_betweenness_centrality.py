...
class TestBetweennessCentrality:
    ...
    def test_scale_with_k_on_star_graph(self, normalized, endpoints, is_directed, k, expected):
        # seed=1 selects node 1 as the initial node when using k=1.
        # Recall node 0 is the center of the star graph.
        G = nx.star_graph(4)
        if is_directed:
            G = G.to_directed()
        b = nx.betweenness_centrality(G, k=k, seed=1, endpoints=endpoints, normalized=normalized)
    ...
    def test_scale_with_k_on_cycle_graph(self, normalized, endpoints, is_directed, k, expected):
