    ...
    def __init__(self, G1, G2):
        ...
        if not is_directed_matcher and (G1.is_directed() or G2.is_directed()):
            raise nx.NetworkXError(
                "(Multi-)GraphMatcher() not defined for directed graphs. " "Use (Multi-)DiGraphMatcher() instead."
            )

        if is_directed_matcher and not (G1.is_directed() and G2.is_directed()):
            raise nx.NetworkXError(
                "(Multi-)DiGraphMatcher() not defined for undirected graphs. " "Use (Multi-)GraphMatcher() instead."
            )
