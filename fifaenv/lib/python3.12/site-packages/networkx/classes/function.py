...
def selfloop_edges(G, data=False, keys=False, default=None):
    ...
    if data is True:
        if G.is_multigraph():
            if keys is True:
                return ((n, n, k, d) for n, nbrs in G._adj.items() if n in nbrs for k, d in nbrs[n].items())
            else:
                return ((n, n, d) for n, nbrs in G._adj.items() if n in nbrs for d in nbrs[n].values())
        else:
        ...
        if G.is_multigraph():
            if keys is True:
                ...
                return (
                    (n, n, d.get(data, default)) for n, nbrs in G._adj.items() if n in nbrs for d in nbrs[n].values()
                )
        else:
