        ...
        *,
        ...
        if G.is_multigraph():
            ...
            def G_new_inner(keydict):
                kd = G.adjlist_inner_dict_factory((k, G_new_datadict(dd)) for k, dd in keydict.items())
                return kd
        ...
        if G.is_directed():
            ...
            for n, nbrs in graph._adj.items():
                ...
                G_adj[n].update(
                    (nbr, G_adj[nbr][n] if n in G_adj[nbr] else G_new_inner(dd)) for nbr, dd in nbrs.items()
                )
