...
def _set_configs_from_environment():
    ...
    config = NetworkXConfig(
        ...
        warnings_to_ignore=set(_comma_sep_to_list(os.environ.get("NETWORKX_WARNINGS_TO_IGNORE", ""))),
    )
...
class _dispatchable:
    ...
    def __new__(
        ...
        *,
        ...
        self.backends = {
            backend for backend, info in backend_info.items() if "functions" in info and name in info["functions"]
        }
    ...
    def _call_if_any_backends_installed(self, /, *args, backend=None, **kwargs):
        ...
        if self.list_graphs:
            ...
            for gname in self.list_graphs & graphs_resolved.keys():
                graph_backend_names.update(getattr(g, "__networkx_backend__", None) for g in graphs_resolved[gname])
        else:
            graph_backend_names = {getattr(g, "__networkx_backend__", None) for g in graphs_resolved.values()}

        backend_priority = nx.config.backend_priority.get(
            self.name,
            nx.config.backend_priority.generators if self._returns_graph else nx.config.backend_priority.algos,
        )
        ...
        if backend_name is not None:
            ...
            if not graph_backend_names or graph_backend_names == {backend_name}:
                ...
                if self._can_backend_run(backend_name, args, kwargs):
                    return self._call_with_backend(backend_name, args, kwargs, extra_message=extra_message)
                if self._does_backend_have(backend_name):
                    extra = " for the given arguments"
                else:
                    extra = ""
                raise NotImplementedError(
                    f"'{self.name}' is not implemented by '{backend_name}' backend" f"{extra}. {backend_kwarg_msg}"
                )
        ...
        if len(try_order) > 1:
            ...
            _logger.debug(
                "Call to '%s' has inputs from %s backends, and will try to use " "backends in the following order: %s",
                self.name,
            ...
            try:
                if not graph_backend_names or graph_backend_names == {backend_name}:
                    ...
                    if self._should_backend_run(backend_name, args, kwargs):
                        ...
                        if self._returns_graph and graph_backend_names and backend_name not in graph_backend_names:
        ...
        if "networkx" not in self.backends:
            extra = " This function is included in NetworkX as an API to dispatch to " "other backends."
        else:
            extra = ""
        raise NotImplementedError(
    ...
    __call__: typing.Callable = _call_if_any_backends_installed if backends else _call_if_no_backends_installed
        ...
        *,
        ...
        if nx_cache is not None:
            ...
            if rv is not None:
                ...
                _logger.debug(
                    "Using cached converted graph (from '%s' to '%s' backend) " "in call to '%s' for '%s' argument",
        ...
        if nx_cache is not None:
            ...
            _logger.debug(
                "Caching converted graph (from '%s' to '%s' backend) " "in call to '%s' for '%s' argument",
                getattr(graph, "__networkx_backend__", None),
                backend_name,
                self.name,
                graph_name,
    ...
    def _convert_and_call(
        ...
        *,
        ...
        _logger.debug(
            ...
            other_backend_names if len(other_backend_names) > 1 else f"'{next(iter(other_backend_names))}'",
    ...
    def _convert_and_call_for_tests(self, backend_name, args, kwargs, *, fallback_to_nx=False):
        ...
        if not self._can_backend_run(backend_name, args, kwargs):
            if fallback_to_nx or not self.graphs:
                if fallback_to_nx:
                    _logger.debug(
                        "Falling back to use 'networkx' instead of '%s' backend " "for call to '%s' with arguments: %s",
                        backend_name,
                        self.name,
                        _LazyArgsRepr(self, args, kwargs),
                    )
        ...
        compare_inputs_to_nx = "networkx" in self.backends and self._will_call_mutate_input(args, kwargs)
        ...
        try:
            ...
            if fallback_to_nx:
                _logger.debug(
                    "Backend '%s' raised when calling '%s': %s; " "falling back to use 'networkx' instead.",
                    backend_name,
                    self.name,
                    exc,
                )
        ...
        if (
            ...
            != (
                ...
                and any(isinstance(x, nx.Graph) or hasattr(x, "__networkx_backend__") for x in result)
            )
            and not (
...
def _get_from_cache(cache, key, *, backend_name=None, mutations=None):
    ...
    for compat_key in itertools.product(
        ...
        if (rv := cache.get(compat_key)) is not None and (rv != FAILED_TO_CONVERT or key == compat_key):
