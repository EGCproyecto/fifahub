...
def spring_layout(
    ...
    *,
    ...
    if len(G) >= 500 or method == "energy":
        ...
        pos = _sparse_fruchterman_reingold(A, k, pos_arr, fixed, iterations, threshold, dim, seed, method, gravity)
    else:
...
def _sparse_fruchterman_reingold(
    ...
    if method == "energy":
        return _energy_fruchterman_reingold(A, nnodes, k, pos, fixed, iterations, threshold, dim, gravity)
...
def _energy_fruchterman_reingold(A, nnodes, k, pos, fixed, iterations, threshold, dim, gravity):
    ...
    return sp.optimize.minimize(_cost_FR, pos.ravel(), method="L-BFGS-B", jac=True, options=options).x.reshape(
        (nnodes, dim)
    )
...
def multipartite_layout(G, subset_key="subset", align="vertical", scale=1, center=None, store_pos_as=None):
