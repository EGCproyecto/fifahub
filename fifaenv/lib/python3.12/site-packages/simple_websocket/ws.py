...
class Base:
    def __init__(
        ...
        self.thread.name = self.thread.name.replace("(_thread)", "(simple_websocket.Base._thread)")
        self.thread.start()
                ...
                if sel:
                    ...
                    if next_ping <= now or not sel.select(next_ping - now):
                        ...
                        if not self.pong_received:
                            self.close(reason=CloseReason.POLICY_VIOLATION, message="Ping/Pong timeout")
                            self.event.set()
                            break
...
class Client(Base):
    ...
    def __init__(
        ...
        connection_args = socket.getaddrinfo(self.host, self.port, type=socket.SOCK_STREAM)
        if len(connection_args) == 0:  # pragma: no cover
            raise ConnectionError()
        sock = socket.socket(connection_args[0][0], connection_args[0][1], connection_args[0][2])
