...
class AioBase:
    def __init__(self, connection_type=None, receive_bytes=4096, ping_interval=None, max_message_size=None):
                    ...
                    if next_ping > now:
                        ...
                        try:
                            in_data = await asyncio.wait_for(
                                self.rsock.read(self.receive_bytes), timeout=next_ping - now
                            )
                        except asyncio.TimeoutError:
                            timed_out = True
                    if timed_out:
                        ...
                        if not self.pong_received:
                            await self.close(reason=CloseReason.POLICY_VIOLATION, message="Ping/Pong timeout")
                            break
...
class AioServer(AioBase):
    ...
    def __init__(self, request, subprotocols=None, receive_bytes=4096, ping_interval=None, max_message_size=None):
        super().__init__(
    ...
    async def accept(
        ...
        if asgi:  # pragma: no cover
            ...
            return await WebSocketASGI.accept(asgi[0], asgi[1], asgi[2], subprotocols=subprotocols)
...
class AioClient(AioBase):
    ...
    async def _connect(self):
        ...
        self.rsock, self.wsock = await asyncio.open_connection(self.host, self.port, ssl=self.ssl_context)
        await super().connect()

    async def handshake(self):
        out_data = self.ws.send(
            Request(host=self.host, target=self.path, subprotocols=self.subprotocols, extra_headers=self.extra_headeers)
        )
        self.wsock.write(out_data)
