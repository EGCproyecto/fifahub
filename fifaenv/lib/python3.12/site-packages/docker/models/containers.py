    ...
    def labels(self):
        ...
        try:
            ...
            raise DockerException(
                "Label data is not available for sparse objects. Call reload()" " to retrieve all information"
            ) from ke
...
class ContainerCollection(Collection):
    ...
    def run(self, image, command=None, stdout=True, stderr=False, remove=False, **kwargs):
        ...
        if detach and remove:
            if version_gte(self.client.api._version, "1.25"):
                ...
                raise RuntimeError(
                    "The options 'detach' and 'remove' cannot " "be used together in api versions < 1.25."
                )
        ...
        if kwargs.get("networking_config") and not kwargs.get("network"):
            raise RuntimeError('The option "networking_config" can not be used ' 'without "network".')
    ...
    def list(self, all=False, before=None, filters=None, limit=-1, since=None, sparse=False, ignore_removed=False):
        """
...
def _create_container_args(kwargs):
    ...
    if network:
        ...
        create_kwargs["networking_config"] = (
            NetworkingConfig(networking_config) if networking_config else {network: None}
        )
