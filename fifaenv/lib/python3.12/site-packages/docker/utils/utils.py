...
def convert_tmpfs_mounts(tmpfs):
    ...
    if not isinstance(tmpfs, list):
        raise ValueError("Expected tmpfs value to be either a list or a dict, " f"found: {type(tmpfs).__name__}")
    ...
    for mount in tmpfs:
        if isinstance(mount, str):
            ...
            raise ValueError("Expected item in tmpfs list to be a string, " f"found: {type(mount).__name__}")
...
def parse_host(addr, is_win32=False, tls=False):
    ...
    if parsed_url.path and proto == "ssh":
        raise errors.DockerException(f"Invalid bind address format: no path allowed for this protocol: {addr}")
    else:
    ...
    for device in devices:
        ...
        if device_mapping:
            ...
            if len(device_mapping) > 2:
                ...
                permissions = "rwm"
            device_list.append(
                {"PathOnHost": path_on_host, "PathInContainer": path_in_container, "CgroupPermissions": permissions}
            )
    return device_list
    ...
    if suffix in units.keys() or suffix.isdigit():
        try:
            ...
            raise errors.DockerException(
                "Failed converting the string value for memory " f"({digits_part}) to an integer."
            ) from ve
