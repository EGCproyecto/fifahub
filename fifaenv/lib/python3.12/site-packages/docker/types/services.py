    ...
    def __init__(
        ...
        if type == "bind":
            ...
            if any([labels, driver_config, no_copy, tmpfs_size, tmpfs_mode]):
                raise errors.InvalidArgument("Incompatible options have been provided for the bind " "type mount.")
        elif type == "volume":
            volume_opts = {}
            if no_copy:
            ...
            if any([propagation, tmpfs_size, tmpfs_mode]):
                raise errors.InvalidArgument("Incompatible options have been provided for the volume " "type mount.")
        elif type == "tmpfs":
            tmpfs_opts = {}
            if tmpfs_mode:
            ...
            if any([propagation, labels, driver_config, no_copy]):
                raise errors.InvalidArgument("Incompatible options have been provided for the tmpfs " "type mount.")
...
class Resources(dict):
    ...
    def __init__(
        self, cpu_limit=None, mem_limit=None, cpu_reservation=None, mem_reservation=None, generic_resources=None
    ):
        limits = {}
        reservation = {}
        ...
        if generic_resources is not None:
            reservation["GenericResources"] = _convert_generic_resources_dict(generic_resources)
        if limits:
            self["Limits"] = limits
        if reservation:
...
def _convert_generic_resources_dict(generic_resources):
    ...
    if not isinstance(generic_resources, dict):
        raise errors.InvalidArgument("generic_resources must be a dict or a list " f"(found {type(generic_resources)})")
    resources = []
    for kind, value in generic_resources.items():
        ...
        if isinstance(value, int):
            ...
            kv = {kind: value}
            raise errors.InvalidArgument(f"Unsupported generic resource reservation type: {kv}")
...
class UpdateConfig(dict):
    ...
    def __init__(
        self, parallelism=0, delay=None, failure_action="continue", monitor=None, max_failure_ratio=None, order=None
    ):
...
class RestartPolicy(dict):
    ...
    def __init__(self, condition=RestartConditionTypesEnum.NONE, delay=0, max_attempts=0, window=0):
...
def convert_service_ports(ports):
    ...
    for k, v in ports.items():
        ...
        if isinstance(v, tuple):
            ...
            if len(v) > 3:
                raise ValueError(
                    "Service port configuration can have at most 3 elements: " "(target_port, protocol, mode)"
                )
        else:
    ...
    def __init__(self, mode, replicas=None, concurrency=None):
        ...
        if mode not in supported_modes:
            raise errors.InvalidArgument(
                'mode must be either "replicated", "global", "replicated-job"' ' or "global-job"'
            )

        if mode not in replicated_modes:
            if replicas is not None:
                raise errors.InvalidArgument('replicas can only be used for "replicated" or' ' "replicated-job" mode')

            if concurrency is not None:
                raise errors.InvalidArgument('concurrency can only be used for "replicated-job" mode')
...
class SecretReference(dict):
    ...
    def __init__(self, secret_id, secret_name, filename=None, uid=None, gid=None, mode=0o444):
...
class ConfigReference(dict):
    ...
    def __init__(self, config_id, config_name, filename=None, uid=None, gid=None, mode=0o444):
...
class PlacementPreference(dict):
    ...
    def __init__(self, strategy, descriptor):
        if strategy != "spread":
            raise errors.InvalidArgument(
                f"PlacementPreference strategy value is invalid ({strategy}): " 'must be "spread".'
            )
...
class Privileges(dict):
    ...
    def __init__(
        ...
        if len(credential_spec) > 1:
            raise errors.InvalidArgument("credentialspec_file and credentialspec_registry are mutually" " exclusive")
