...
class EndpointConfig(dict):
    def __init__(
        ...
        if version_lt(version, "1.22"):
            raise errors.InvalidVersion("Endpoint config is not supported for API version < 1.22")
        ...
        if mac_address:
            if version_lt(version, "1.25"):
                raise errors.InvalidVersion("mac_address is not supported for API version < 1.25")
            self["MacAddress"] = mac_address

        if link_local_ips is not None:
            if version_lt(version, "1.24"):
                raise errors.InvalidVersion("link_local_ips is not supported for API version < 1.24")
        ...
        if driver_opt:
            if version_lt(version, "1.32"):
                raise errors.InvalidVersion("DriverOpts is not supported for API version < 1.32")
