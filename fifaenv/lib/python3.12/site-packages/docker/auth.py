...
def resolve_repository_name(repo_name):
    if "://" in repo_name:
        raise errors.InvalidRepository(f"Repository name cannot contain a scheme ({repo_name})")

    index_name, remote_name = split_repo_name(repo_name)
    if index_name[0] == "-" or index_name[-1] == "-":
        raise errors.InvalidRepository(f"Invalid index name ({index_name}). " "Cannot begin or end with a hyphen.")
...
class AuthConfig(dict):
    ...
    def parse_auth(cls, entries, raise_on_error=False):
        ...
        for registry, entry in entries.items():
            if not isinstance(entry, dict):
                ...
                if raise_on_error:
                    raise errors.InvalidConfigFile(f"Invalid configuration for registry {registry}")
                return {}
            ...
            if "auth" not in entry:
                ...
                log.debug(f"Auth data for {registry} is absent. " f"Client might be using a credentials store instead.")
                conf[registry] = {}
                continue

            username, password = decode_auth(entry["auth"])
            log.debug(f"Found entry (registry={registry!r}, username={username!r})")
    ...
    def _resolve_authconfig_credstore(self, registry, credstore_name):
        ...
        try:
            ...
            raise errors.DockerException(f"Credentials store error: {repr(e)}") from e

    def _get_store_instance(self, name):
        if name not in self._stores:
            self._stores[name] = credentials.Store(name, environment=self._credstore_env)
