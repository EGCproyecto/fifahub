...
class BuildApiMixin:
    def build(
        ...
        if custom_context:
            ...
            if os.path.exists(dockerignore):
                with open(dockerignore) as f:
                    exclude = list(
                        filter(lambda x: x != "" and x[0] != "#", [line.strip() for line in f.read().splitlines()])
                    )
        ...
        if shmsize:
            if utils.version_gte(self._version, "1.22"):
                ...
                raise errors.InvalidVersion("shmsize was only introduced in API version 1.22")

        if labels:
            if utils.version_gte(self._version, "1.23"):
                params.update({"labels": json.dumps(labels)})
            else:
                raise errors.InvalidVersion("labels was only introduced in API version 1.23")

        if cache_from:
            if utils.version_gte(self._version, "1.25"):
                params.update({"cachefrom": json.dumps(cache_from)})
            else:
                raise errors.InvalidVersion("cache_from was only introduced in API version 1.25")

        if target:
            if utils.version_gte(self._version, "1.29"):
                params.update({"target": target})
            else:
                raise errors.InvalidVersion("target was only introduced in API version 1.29")

        if network_mode:
            if utils.version_gte(self._version, "1.25"):
                params.update({"networkmode": network_mode})
            else:
                raise errors.InvalidVersion("network_mode was only introduced in API version 1.25")

        if squash:
            if utils.version_gte(self._version, "1.25"):
                params.update({"squash": squash})
            else:
                raise errors.InvalidVersion("squash was only introduced in API version 1.25")

        if extra_hosts is not None:
            if utils.version_lt(self._version, "1.27"):
                raise errors.InvalidVersion("extra_hosts was only introduced in API version 1.27")
        ...
        if platform is not None:
            if utils.version_lt(self._version, "1.32"):
                raise errors.InvalidVersion("platform was only introduced in API version 1.32")
            params["platform"] = platform

        if isolation is not None:
            if utils.version_lt(self._version, "1.24"):
                raise errors.InvalidVersion("isolation was only introduced in API version 1.24")
    ...
    def prune_builds(self, filters=None, keep_storage=None, all=None):
        ...
        if (filters, keep_storage, all) != (None, None, None) and utils.version_lt(self._version, "1.39"):
            raise errors.InvalidVersion(
                "`filters`, `keep_storage`, and `all` args are only available " "for API version > 1.38"
            )
        params = {}
...
def process_dockerfile(dockerfile, path):
    ...
    if not os.path.isabs(dockerfile):
        ...
        if constants.IS_WINDOWS_PLATFORM and path.startswith(constants.WINDOWS_LONGPATH_PREFIX):
            normpath = os.path.normpath(abs_dockerfile[len(constants.WINDOWS_LONGPATH_PREFIX) :])
            abs_dockerfile = f"{constants.WINDOWS_LONGPATH_PREFIX}{normpath}"
    if os.path.splitdrive(path)[0] != os.path.splitdrive(abs_dockerfile)[0] or os.path.relpath(
        abs_dockerfile, path
    ).startswith(".."):
