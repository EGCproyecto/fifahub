...
class APIError(requests.exceptions.HTTPError, DockerException):
    ...
    def __str__(self):
        ...
        if self.is_client_error():
            message = f"{self.response.status_code} Client Error for " f"{self.response.url}: {self.response.reason}"

        elif self.is_server_error():
            message = f"{self.response.status_code} Server Error for " f"{self.response.url}: {self.response.reason}"
...
class ContainerError(DockerException):
    ...
    def __init__(self, container, exit_status, command, image, stderr):
        ...
        super().__init__(f"Command '{command}' in image '{image}' " f"returned non-zero exit status {exit_status}{err}")
