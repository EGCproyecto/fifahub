...
actually_imported = copy_globals(__subprocess__, globals(), only_names=__imports__, ignore_missing_names=True)
    ...
    def __init__(
        ...
        if p2cwrite != -1:
            if text_mode:
                ...
                self.stdin = FileObject(p2cwrite, "w", bufsize, encoding=self.encoding, errors=self.errors)
            else:
        ...
        if extra_groups is not None:
            ...
            for extra_group in extra_groups:
                if isinstance(extra_group, str):
                    ...
                    raise TypeError(
                        "Items in extra_groups must be a string " "or integer, not {}".format(type(extra_group))
                    )
                ...
                if have_handle_list or (use_std_handles and close_fds):
                    ...
                    if handle_list:
                        if not close_fds:
                            ...
                            warnings.warn(
                                "startupinfo.lpAttributeList['handle_list'] " "overriding close_fds", RuntimeWarning
                            )
