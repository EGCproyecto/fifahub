...
class WatcherTestMixin(object):
    ...
    def test_events_conversion(self):
        self.assertEqual(
            self.core._events_to_str(self.core.READ | self.core.WRITE), "READ|WRITE"  # pylint: disable=no-member
        )

    def test_EVENTS(self):
        self.assertEqual(str(self.core.EVENTS), "gevent.core.EVENTS")  # pylint: disable=no-member
        self.assertEqual(repr(self.core.EVENTS), "gevent.core.EVENTS")  # pylint: disable=no-member
    ...
    def test_io(self):
        ...
        if not greentest.WIN:
            ...
            if not hasattr(self.core, "libuv"):
                ...
                self.assertEqual(self.core._events_to_str(io.events), "WRITE")  # pylint:disable=no-member
