...
class AbstractTestMixin(object):
    ...
    def skipIfNoAll(self):
        if not hasattr(self.module, "__all__"):
            self.assertTrue(self.modname in NO_ALL or self.modname.startswith("gevent.monkey."), "Module has no all")
    ...
    def test_all_formula(self):
        ...
        all_calculated = (
            tuple(self.__implements__ or ()) + tuple(self.__imports__ or ()) + tuple(self.__extensions__ or ())
        )
        try:
            ...
            raise AssertionError(
                "Unable to sort %r from all %s in %s" % (all_calculated, self.module.__all__, self.module)
            )
    ...
    def test_implements_subset_of_stdlib_all(self):
        ...
        for name in tuple(self.__implements__ or ()) + tuple(self.__imports__):
            ...
            if name in dir(self.stdlib_module):  # like thread._local which is not in thread.__all__
                continue
            raise AssertionError(
                "%r is not found in %r.__all__ nor in dir(%r)" % (name, self.stdlib_module, self.stdlib_module)
            )
...
def _create_tests():
    for _, modname in modules.walk_modules(include_so=False, recursive=True, check_optional=False):
        if modname.endswith(PLATFORM_SPECIFIC_SUFFIXES):
            continue
