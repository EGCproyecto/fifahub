...
class Definitions(metaclass=DefinitionsMeta):
    ...
    test_subprocess = (
        ...
        .ignored("Tests don't even start before the process times out.", when=PYPY3 & WIN)
    )
    ...
    test__refcount = Ignored(
        "Sometimes fails to connect for no reason", when=(CI & OSX) | (CI & PYPY) | APPVEYOR, ignore_coverage=PYPY
    )
