        ...
        if isinstance(o, rlock_type):
            for owner_name in (
                ...
                raise AssertionError(
                    "Unsupported Python implementation; " "Found unknown lock implementation.", vars(o)
                )
...
class BasePatcher:
    ...
    def __init__(
        self, threading=True, _threading_local=True, Event=True, logging=True, existing_locks=True, _warnings=None
    ):
    ...
    def __call__(self):
        ...
        gevent_thread_mod, thread_mod = _patch_module("thread", _warnings=self.warnings, _notify_did_subscribers=False)
        ...
        if self.gevent_threading_mod is not None:
            _notify_patch(events.GeventDidPatchModuleEvent("threading", self.gevent_threading_mod, self.threading_mod))
