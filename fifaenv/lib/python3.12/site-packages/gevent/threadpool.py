...
def _format_hub(hub):
    if hub is None:
        return "<missing>"
    return "<%s at 0x%x thread_ident=0x%x>" % (hub.__class__.__name__, id(hub), hub.thread_ident)
...
class _WorkerGreenlet(RawGreenlet):
    ...
    def run(self):
        ...
        try:  # pylint:disable=too-many-nested-blocks
            ...
            print("Failed to run worker thread. Task=%r Exception=%r" % (task, e), file=self._stderr)
...
class ThreadPool(GroupMappingMixin):
    ...
    def _adjust_step(self):
        ...
        while len(self._worker_greenlets) < self._maxsize and self.task_queue.unfinished_tasks > len(
