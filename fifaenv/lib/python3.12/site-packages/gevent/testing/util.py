...
def run(command, **kwargs):  # pylint:disable=too-many-locals
    ...
    if result:
        log("! %s [code %s] %s", name, result, status, color="error" if failed else "suboptimal-behaviour")
    elif not nested:
        log("- %s %s", name, status)
...
class ExampleMixin(object):
    ...
    def _start(self, meth):
        ...
        return meth([sys.executable, "-W", "ignore", "-u", server] + self.example_args, cwd=server_dir, **kwargs)
