...
class signal(object):
    ...
    ref = property(lambda self: self.watcher.ref, lambda self, nv: setattr(self.watcher, "ref", nv))
...
class Hub(WaitOperationsGreenlet):
    ...
    def __repr__(self):
        ...
        result = "<%s %r at 0x%x %s" % (self.__class__.__name__, self.name, id(self), info)
    ...
    def start_periodic_monitoring_thread(self):
        if self.periodic_monitoring_thread is None and GEVENT_CONFIG.monitor_thread:
            ...
            from gevent.events import PeriodicMonitorThreadStartedEvent, notify_and_call_entry_points
            ...
            notify_and_call_entry_points(PeriodicMonitorThreadStartedEvent(self.periodic_monitoring_thread))
    ...
    def _get_threadpool(self):
        if self._threadpool is None:
            ...
            self._threadpool = self.threadpool_class(
                self.threadpool_size, hub=self, idle_task_timeout=GEVENT_CONFIG.threadpool_idle_task_timeout
            )
