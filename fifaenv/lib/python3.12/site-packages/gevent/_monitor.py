...
class PeriodicMonitoringThread(object):
    ...
    def __init__(self, hub):
        ...
        self._monitoring_functions = [_MonitorEntry(self.monitor_blocking, GEVENT_CONFIG.max_blocking_time)]
    ...
    def add_monitoring_function(self, function, period):
        ...
        if period is None:
            # Remove.
            self._monitoring_functions = [x for x in self._monitoring_functions if x.function != function]
        elif period <= 0:
            raise ValueError("Period must be positive.")
        else:
            # Add or update period
            entry = _MonitorEntry(function, period)
            self._monitoring_functions = [x if x.function != function else entry for x in self._monitoring_functions]
    ...
    def monitor_blocking(self, hub):
        ...
        report = self._greenlet_tracer.did_block_hub_report(
            hub, active_greenlet, dict(greenlet_stacks=False, current_thread_ident=self.monitor_thread_ident)
        )

        # Notify the event first. ``report`` is a mutable list, so the event listeners
        # may mutate it to add or remove information.
        notify(EventLoopBlocked(active_greenlet, GEVENT_CONFIG.max_blocking_time, report, hub=hub))
    ...
    def install_monitor_memory_usage(self):
        ...
        if not self.can_monitor_memory_usage():
            ...
            warnings.warn("Unable to monitor memory usage. Install psutil.", MonitorWarning)
            return

        self.add_monitoring_function(
            self.monitor_memory_usage, max(GEVENT_CONFIG.memory_monitor_period, self.min_memory_monitor_period)
        )
    ...
    def monitor_memory_usage(self, _hub):
        ...
        if mem_usage > max_allowed:
            ...
            if self._memory_exceeded:
                event = MemoryUsageUnderThreshold(mem_usage, max_allowed, rusage, self._memory_exceeded)
                notify(event)
