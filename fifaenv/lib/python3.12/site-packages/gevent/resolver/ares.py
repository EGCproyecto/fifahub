...
from gevent.socket import AF_INET, AF_INET6, AF_UNSPEC, SOCK_DGRAM, SOCK_STREAM, SOL_TCP, SOL_UDP
...
class Resolver(AbstractResolver):
    ...
    def __getaddrinfo(self, host, port, family=0, socktype=0, proto=0, flags=0, fill_in_type_proto=True):
        ...
        if fill_in_type_proto:
            ...
            result = [
                (rfamily, hard_type if not rtype else rtype, hard_proto if not rproto else rproto, rcanon, raddr)
    ...
    def __getnameinfo(self, hostname, port, sockaddr, flags):
        result = self.__getaddrinfo(
            hostname, port, family=AF_UNSPEC, socktype=SOCK_DGRAM, proto=0, flags=0, fill_in_type_proto=False
        )
        if len(result) != 1:
