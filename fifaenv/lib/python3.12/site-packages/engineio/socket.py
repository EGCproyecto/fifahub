...
class Socket(base_socket.BaseSocket):
    ...
    def poll(self):
        ...
        try:
            packets = [self.queue.get(timeout=self.server.ping_interval + self.server.ping_timeout)]
    ...
    def receive(self, pkt):
        ...
        if pkt.packet_type == packet.PONG:
            ...
            self.close(wait=False, abort=True, reason=self.server.reason.CLIENT_DISCONNECT)
        else:
    ...
    def check_ping_timeout(self):
        ...
        if self.last_ping and time.time() - self.last_ping > self.server.ping_timeout:
            ...
            self.close(wait=False, abort=False, reason=self.server.reason.PING_TIMEOUT)
            return False
        return True
    ...
    def _websocket_handler(self, ws):
        ...
        for attr in ["_sock", "socket"]:  # pragma: no cover
            if hasattr(ws, attr) and hasattr(getattr(ws, attr), "settimeout"):
                getattr(ws, attr).settimeout(self.server.ping_interval + self.server.ping_timeout)
        ...
        writer_task.join()
        self.close(wait=False, abort=True, reason=self.server.reason.TRANSPORT_CLOSE)
