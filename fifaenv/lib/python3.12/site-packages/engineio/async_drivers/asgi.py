...
class ASGIApp:
    ...
    async def __call__(self, scope, receive, send):
        if scope["type"] == "lifespan":
            ...
            self.engineio_path is None or self._ensure_trailing_slash(scope["path"]).startswith(self.engineio_path)
        ):
            ...
            await send(
                {
                    ...
                    "status": 200,
                    "headers": [(b"Content-Type", static_file["content_type"].encode("utf-8"))],
                }
            )
            await send({"type": "http.response.body", "body": payload})

    async def lifespan(self, scope, receive, send):
        if self.other_asgi_app is not None and self.on_startup is None and self.on_shutdown is None:
...
async def make_response(status, headers, payload, environ):
    ...
    if environ["asgi.scope"]["type"] == "websocket":
        if status.startswith("200 "):
            ...
            if payload:
                reason = payload.decode("utf-8") if isinstance(payload, bytes) else str(payload)
                await environ["asgi.send"]({"type": "websocket.close", "reason": reason})
            else:
                await environ["asgi.send"]({"type": "websocket.close"})
        return

    await environ["asgi.send"]({"type": "http.response.start", "status": int(status.split(" ")[0]), "headers": headers})
...
class WebSocket:  # pragma: no cover
    ...
    async def send(self, message):
        ...
        if isinstance(message, bytes):
            ...
            msg_text = message
        await self.asgi_send({"type": "websocket.send", "bytes": msg_bytes, "text": msg_text})
