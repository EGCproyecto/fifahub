...
from . import base_socket, exceptions, packet, payload
...
class AsyncSocket(base_socket.BaseSocket):
    async def poll(self):
        ...
        try:
            packets = [await asyncio.wait_for(self.queue.get(), self.server.ping_interval + self.server.ping_timeout)]
    ...
    async def receive(self, pkt):
        ...
        if pkt.packet_type == packet.PONG:
            ...
            await self.server._trigger_event("message", self.sid, pkt.data, run_async=self.server.async_handlers)
        elif pkt.packet_type == packet.UPGRADE:
            await self.send(packet.Packet(packet.NOOP))
        elif pkt.packet_type == packet.CLOSE:
            await self.close(wait=False, abort=True, reason=self.server.reason.CLIENT_DISCONNECT)
        else:
    ...
    async def check_ping_timeout(self):
        ...
        if self.last_ping and time.time() - self.last_ping > self.server.ping_timeout:
            ...
            await self.close(wait=False, abort=False, reason=self.server.reason.PING_TIMEOUT)
            return False
        return True
    ...
    async def handle_get_request(self, environ):
        ...
        try:
            ...
            exc = sys.exc_info()
            await self.close(wait=False, reason=self.server.reason.TRANSPORT_ERROR)
    ...
    async def close(self, wait=True, abort=False, reason=None):
        ...
        if not self.closed and not self.closing:
            ...
            await self.server._trigger_event(
                "disconnect", self.sid, reason or self.server.reason.SERVER_DISCONNECT, run_async=False
            )
            if not abort:
    ...
    async def _websocket_handler(self, ws):
        ...
        while True:
            ...
            try:
                p = await asyncio.wait_for(wait_task, self.server.ping_interval + self.server.ping_timeout)
        ...
        await self.close(wait=False, abort=True, reason=self.server.reason.TRANSPORT_CLOSE)
