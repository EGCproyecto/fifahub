...
from . import base_client, exceptions, packet, payload
...
class AsyncClient(base_client.BaseClient):
    ...
    async def connect(self, url, headers=None, transports=None, engineio_path="engine.io"):
        ...
        if (
            ...
            try:
                asyncio.get_running_loop().add_signal_handler(signal.SIGINT, async_signal_handler)
        ...
        return await getattr(self, "_connect_" + self.transports[0])(url, headers or {}, engineio_path)
    ...
    async def disconnect(self, abort=False, reason=None):
        ...
        if self.state == "connected":
            ...
            await self._trigger_event("disconnect", reason or self.reason.CLIENT_DISCONNECT, run_async=False)
    ...
    async def _connect_polling(self, url, headers, engineio_path):
        ...
        r = await self._send_request(
            "GET", self.base_url + self._get_url_timestamp(), headers=headers, timeout=self.request_timeout
        )
        ...
        if r.status < 200 or r.status >= 300:
            ...
            try:
                ...
                arg = None
            raise exceptions.ConnectionError("Unexpected status code {} in server response".format(r.status), arg)
        try:
            p = payload.Payload(encoded_payload=(await r.read()).decode("utf-8"))
        except ValueError:
            raise exceptions.ConnectionError("Unexpected response from server") from None
    ...
    async def _connect_websocket(self, url, headers, engineio_path):
        ...
        try:
            ws = await self.http.ws_connect(websocket_url + self._get_url_timestamp(), **extra_options)
        except (
    ...
    async def _receive_packet(self, pkt):
        ...
        self.logger.info(
            "Received packet %s data %s", packet_name, pkt.data if not isinstance(pkt.data, bytes) else "<binary>"
        )
    ...
    async def _send_request(self, method, url, headers=None, body=None, timeout=None):  # pragma: no cover
        ...
        try:
            if not self.ssl_verify:
                return await http_method(
                    url, headers=headers, data=body, timeout=aiohttp.ClientTimeout(total=timeout), ssl=False
                )
            else:
                return await http_method(url, headers=headers, data=body, timeout=aiohttp.ClientTimeout(total=timeout))
                ...
                if run_async:
                    ...
                    try:
                        try:
                            ...
                            if event == "disconnect" and len(args) == 1:  # pragma: no branch
    ...
    async def _read_loop_polling(self):
        ...
        while self.state == "connected" and self.write_loop_task:
            ...
            r = await self._send_request(
                "GET", self.base_url + self._get_url_timestamp(), timeout=max(self.ping_interval, self.ping_timeout) + 5
            )
            if r is None or isinstance(r, str):
        ...
        if self.state == "connected":
            await self._trigger_event("disconnect", self.reason.TRANSPORT_ERROR, run_async=False)
            try:
                base_client.connected_clients.remove(self)
            except ValueError:  # pragma: no cover
                pass
    ...
    async def _read_loop_websocket(self):
        ...
        while self.state == "connected":
            p = None
            try:
                p = await asyncio.wait_for(self.ws.receive(), timeout=self.ping_interval + self.ping_timeout)
                if not isinstance(p.data, (str, bytes)):  # pragma: no cover
                    self.logger.warning(
                        "Server sent %s packet data %s, aborting",
                        "close" if p.type in [aiohttp.WSMsgType.CLOSE, aiohttp.WSMsgType.CLOSING] else str(p.type),
                        str(p.data),
                    )
                    await self.queue.put(None)
                ...
                self.logger.info('Unexpected error receiving packet: "%s", aborting', str(e))
                await self.queue.put(None)
                break
            try:
                pkt = packet.Packet(encoded_packet=p)
            except Exception as e:  # pragma: no cover
                self.logger.info('Unexpected error decoding packet: "%s", aborting', str(e))
                await self.queue.put(None)
                break
            await self._receive_packet(pkt)
            ...
            if self.current_transport == "polling":
                ...
                try:
                    for pkt in packets:
                        ...
                        self.queue.task_done()
                except (aiohttp.client_exceptions.ServerDisconnectedError, BrokenPipeError, OSError):
