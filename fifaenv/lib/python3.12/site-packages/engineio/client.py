...
class Client(base_client.BaseClient):
    ...
    def connect(self, url, headers=None, transports=None, engineio_path="engine.io"):
        ...
        return getattr(self, "_connect_" + self.transports[0])(url, headers or {}, engineio_path)
    ...
    def disconnect(self, abort=False, reason=None):
        ...
        if self.state == "connected":
            ...
            self._trigger_event("disconnect", reason or self.reason.CLIENT_DISCONNECT, run_async=False)
    ...
    def _connect_polling(self, url, headers, engineio_path):
        ...
        if r.status_code < 200 or r.status_code >= 300:
            ...
            try:
                ...
                arg = None
            raise exceptions.ConnectionError("Unexpected status code {} in server response".format(r.status_code), arg)
        try:
    ...
    def _connect_websocket(self, url, headers, engineio_path):
        ...
        if websocket is None:  # pragma: no cover
            # not installed
            self.logger.error("websocket-client package not installed, only " "polling transport is available")
            return False
        ...
        if self.http:
            ...
            if "Authorization" not in headers and self.http.auth is not None:
                ...
                basic_auth = "{}:{}".format(self.http.auth[0], self.http.auth[1]).encode("utf-8")
            ...
            if isinstance(self.http.cert, tuple):
                extra_options["sslopt"] = {"certfile": self.http.cert[0], "keyfile": self.http.cert[1]}
            elif self.http.cert:
            ...
            if self.http.proxies:
                ...
                if websocket_url.startswith("ws://"):
                    proxy_url = self.http.proxies.get("ws", self.http.proxies.get("http"))
                else:  # wss://
                    proxy_url = self.http.proxies.get("wss", self.http.proxies.get("https"))
                if proxy_url:
                    parsed_url = urllib.parse.urlparse(proxy_url if "://" in proxy_url else "scheme://" + proxy_url)
        ...
        try:
            ws = websocket.create_connection(websocket_url + self._get_url_timestamp(), **extra_options)
    ...
    def _send_request(self, method, url, headers=None, body=None, timeout=None):  # pragma: no cover
        ...
        try:
            return self.http.request(method, url, headers=headers, data=body, timeout=timeout)
    ...
    def _read_loop_polling(self):
        ...
        if self.state == "connected":
            self._trigger_event("disconnect", self.reason.TRANSPORT_ERROR, run_async=False)
            try:
                base_client.connected_clients.remove(self)
            except ValueError:  # pragma: no cover
                pass
        ...
        while self.state == "connected":
            ...
            try:
                ...
                if type(e) is OSError and e.errno == 9:
                    self.logger.info("WebSocket connection is closing, aborting")
                else:
