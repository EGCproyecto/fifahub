...
from . import async_socket, base_server, exceptions, packet
...
class AsyncServer(base_server.BaseServer):
    ...
    async def disconnect(self, sid=None):
        ...
        if sid is not None:
            ...
            await asyncio.wait(
                [
                    asyncio.create_task(client.close(reason=self.reason.SERVER_DISCONNECT))
    ...
    async def handle_request(self, *args, **kwargs):
        ...
        if self.cors_allowed_origins != []:
            ...
            if origin:
                ...
                if allowed_origins is not None and origin not in allowed_origins:
                    self._log_error_once(origin + " is not an accepted origin.", "bad-origin")
                    return await self._make_response(self._bad_request(origin + " is not an accepted origin."), environ)
        ...
        if transport not in self.transports:
            ...
            return await self._make_response(self._bad_request("Invalid transport"), environ)

        # make sure the client speaks a compatible Engine.IO version
        sid = query["sid"][0] if "sid" in query else None
        if sid is None and query.get("EIO") != ["4"]:
            self._log_error_once(
                "The client is using an unsupported version of the Socket.IO " "or Engine.IO protocols", "bad-version"
            )
        ...
        if jsonp and jsonp_index is None:
            ...
            upgrade_header = environ.get("HTTP_UPGRADE").lower() if "HTTP_UPGRADE" in environ else None
            if sid is None:
                ...
                if sid not in self.sockets:
                    ...
                    try:
                        ...
                        if self.transport(sid) != transport and transport != upgrade_header:
                            self._log_error_once(f"Invalid transport for session {sid}", "bad-transport")
    ...
    async def _make_response(self, response_dict, environ):
        ...
        if asyncio.iscoroutinefunction(make_response):
            response = await make_response(
                response_dict["status"], response_dict["headers"] + cors_headers, response_dict["response"], environ
            )
        else:
    ...
    async def _handle_connect(self, environ, transport, jsonp_index=None):
        ...
        pkt = packet.Packet(
            packet.OPEN,
            {
                ...
                "pingInterval": int(self.ping_interval + self.ping_interval_grace_period) * 1000,
        ...
        if transport == "websocket":
            ...
            if self.cookie:
                if isinstance(self.cookie, dict):
                    headers = [("Set-Cookie", self._generate_sid_cookie(sid, self.cookie))]
                else:
                    headers = [
                        (
                            "Set-Cookie",
                            self._generate_sid_cookie(sid, {"name": self.cookie, "path": "/", "SameSite": "Lax"}),
                        )
                    ]
            try:
                return self._ok(await s.poll(), headers=headers, jsonp_index=jsonp_index)
                ...
                async def run_async_handler():
                    try:
                        try:
                            ...
                            if event == "disconnect" and len(args) == 2:  # pragma: no branch
    ...
    async def _service_task(self):  # pragma: no cover
        ...
        while not self.service_task_event.is_set():
            if len(self.sockets) == 0:
                # nothing to do
                try:
                    await asyncio.wait_for(self.service_task_event.wait(), timeout=self.ping_timeout)
                    break
            ...
            try:
                ...
                for s in self.sockets.copy().values():
                    ...
                    try:
                        await asyncio.wait_for(self.service_task_event.wait(), timeout=sleep_interval)
