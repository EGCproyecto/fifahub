    ...
    def from_json(cls, json):
        return cls(
            ...
            automatic_presence_simulation=(
                bool(json["automaticPresenceSimulation"]) if "automaticPresenceSimulation" in json else None
            ),
            is_user_verified=bool(json["isUserVerified"]) if "isUserVerified" in json else None,
            default_backup_eligibility=(
                bool(json["defaultBackupEligibility"]) if "defaultBackupEligibility" in json else None
            ),
...
class CredentialDeleted:
    ...
    def from_json(cls, json: T_JSON_DICT) -> CredentialDeleted:
        return cls(
            authenticator_id=AuthenticatorId.from_json(json["authenticatorId"]), credential_id=str(json["credentialId"])
        )
