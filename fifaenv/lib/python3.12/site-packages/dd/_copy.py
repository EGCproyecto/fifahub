...
def dump_json(nodes, file_name):
    ...
    try:
        with shelve.open(tmp_fname) as cache, open(file_name, "w") as f:
...
def _dump_bdd_info(nodes, f):
    ...
    u = next(iter(nodes))
    bdd = u.bdd
    var_level = {var: bdd.level_of_var(var) for var in bdd.vars}
    roots = [_node_to_int(u) for u in nodes]
    s = ('\n"level_of_var": {level}' ',\n"roots": {roots}').format(level=json.dumps(var_level), roots=json.dumps(roots))
    f.write(s)
...
def _dump_bdd(u, f, cache):
    ...
    # dump node
    s = ',\n"{k}": [{level}, {low}, {high}]'.format(k=k, level=u.level, low=low, high=high)
    f.write(s)
    # record as dumped
    cache[str(k)] = True
    return -k if u.negated else k
...
def load_json(file_name, bdd, load_order=False):
    ...
    os.makedirs(SHELVE_DIR)
    try:
        with shelve.open(tmp_fname) as cache, open(file_name, "r") as f:
...
def _load_json(f, bdd, load_order, cache):
    ...
    # the assertion below
    if load_order:
        old_reordering = bdd.configure(reordering=False)
    for line in f:
        d = _parse_line(line)
        _store_line(d, bdd, context, cache)
    roots = [_node_from_int(k, bdd, cache) for k in context["roots"]]
    ...
    if load_order:
        bdd.configure(reordering=old_reordering)
    return roots
...
def _store_line(d, bdd, context, cache):
    ...
    if order is not None:
        ...
        context["var_at_level"] = {v: k for k, v in order.items()}
