...
class BDD(_abc.BDD):
    ...
    def __str__(self):
        return (
            ...
            "\t {n} nodes\n"
        ).format(n_vars=len(self.vars), n=len(self))
    ...
    def let(self, definitions, u):
        ...
        if isinstance(value, Function):
            d = {var: value.node for var, value in d.items()}
...
def image(trans, source, rename, qvars, forall=False):
    ...
    u = _bdd.image(trans.node, source.node, rename, qvars, trans.manager, forall)
    return trans.bdd._wrap(u)


def preimage(trans, target, rename, qvars, forall=False):
    assert trans.bdd is target.bdd
    u = _bdd.preimage(trans.node, target.node, rename, qvars, trans.manager, forall)
    return trans.bdd._wrap(u)
