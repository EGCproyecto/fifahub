...
class MDD(object):
    ...
    def var_at_level(self, i):
        ...
        if self._level_to_var is None:
            self._level_to_var = {d["level"]: var for var, d in items(self.vars)}
    ...
    def ite(self, g, u, v):
        ...
        nodes = tuple(self.ite(a, b, c) for a, b, c in zip(gc, uc, vc))
    ...
    def find_or_add(self, i, *nodes):
        ...
        assert len(nodes) == self.vars[var]["len"], (var, len(nodes), self.vars[var]["len"])
    ...
    def to_expr(self, u):
        ...
        s += ", ".join("\nelif ({var} {j}): {p}".format(var=var, j=cond_str[x], p=e[x]) for x in c[1:])
        if u < 0:
            s = "! {s}".format(s=s)
        s = "({s})".format(s=s)
        return s
    ...
    def apply(self, op, u, v=None, w=None):
        ...
        if op in ("~", "not", "!"):
            ...
            assert v is not None, v
            assert w is None, w
            raise NotImplementedError("quantification is not implemented for MDDs.")
    ...
    def dump(self, fname):
        ...
        if fname.endswith(".pdf"):
            ...
            raise Exception("unknown file extension: {f}".format(f=fname))
