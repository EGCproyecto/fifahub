...
from dd import _abc, _compat, _parser
...
class BDD(_abc.BDD):
    ...
    def __del__(self):
        ...
        if not all(v == 0 for v in self._ref.values()):
            raise AssertionError(
                ("There are nodes still referenced " "upon shutdown. Details:\n" "{_ref}").format(_ref=self._ref)
            )
    ...
    def configure(self, **kw):
        ...
        """
        d = dict(reordering=(self._last_len is not None))
        for k, v in kw.items():
            if k == "reordering":
                if v:
                    self._last_len = max(REORDER_STARTS, len(self))
                else:
                    self._last_len = None
            else:
    ...
    @property
    def ordering(self):
        raise DeprecationWarning("use `dd.bdd.BDD.vars` instead of `.ordering`")
    ...
    def add_var(self, var, level=None):
        ...
        try:
            ...
            other = None
        assert other is None, ("level {level} occupied by {var}, " "choose another level").format(
    ...
    @_try_to_reorder
    def var(self, var):
        assert var in self.vars, ('undefined variable "{v}", ' "known variables are:\n {d}").format(v=var, d=self.vars)
        j = self.vars[var]
    ...
    def var_at_level(self, level):
        if level not in self._level_to_var:
            raise AssertionError("level {j} does not exist".format(j=level))
    ...
    def _map_to_level(self, d):
        ...
        if isinstance(d, Mapping):
            r = {self.vars[var]: bool(val) for var, val in items(d)}
        else:
    ...
    def undeclare_vars(self, *vrs):
        ...
        if vrs:
            full_levels |= {level for var, level in items(self.vars) if var not in vrs}
        ...
        rm_vars = {var for var, level in items(self.vars) if level not in full_levels}
        self.vars = {var: new_levels[old] for var, old in items(self.vars) if old in full_levels}
        self._level_to_var = {k: var for var, k in items(self.vars)}
        # update node levels
        self._succ = {u: (new_levels[i], v, w) for u, (i, v, w) in items(self._succ)}
    ...
    def let(self, definitions, u):
        d = definitions
        if not d:
            logger.warning("Call to `BDD.let` with no effect: " "`defs` is empty.")
            return u
        ...
        try:
            ...
            raise ValueError("Key must be var name as `str`, " "or Boolean value as `bool`, " "or BDD node as `int`.")
    ...
    def compose(self, f, var_sub):
        ...
        if len(var_sub) == 1:
            ...
            dvars = {self.level_of_var(var): g for var, g in var_sub.items()}
    ...
    def _ite(self, g, u, v):
        ...
        if w is not None:
            return w
        z = min(self._succ[abs(g)][0], self._succ[abs(u)][0], self._succ[abs(v)][0])
        g0, g1 = self._top_cofactor(g, z)
    ...
    def swap(self, x, y, all_levels=None):
        ...
        logger.debug('swap variables "{x}" and "{y}"'.format(x=x, y=y))
        x = self.vars.get(x, x)
        y = self.vars.get(y, y)
        ...
        for u, (v, w) in items(levels[x]):
            ...
            assert q >= 0, q
            assert p != q, "No elimination: node depends on both x and y"
    ...
    def count(self, u, nvars=None):
        ...
        levels = {self.level_of_var(var) for var in self.support(u)}
        k = len(levels)
        if n is None:
            n = k
        slack = n - k
    ...
    def pick_iter(self, u, care_vars=None):
        ...
        if missing:
            logger.warning(("Missing bits:  " "support - care_vars = {missing}").format(missing=missing))
    ...
    def _sat_iter(self, u, cube, value):
        ...
        if abs(u) == 1:
            if value:
                cube = {self._level_to_var[i]: v for i, v in items(cube)}
                yield cube
            return
        # non-terminal
        i, v, w = self._succ[abs(u)]
    ...
    def assert_consistent(self):
        ...
        assert succ_keys == pred_values, succ_keys.symmetric_difference(pred_values)
        assert pred_keys == succ_values, pred_keys.symmetric_difference(succ_values)
        # uniqueness
        n = len(succ_keys)
        n_ = len(succ_values)
        assert n == n_, n - n_
    ...
    def dump(self, filename, roots=None, filetype=None, **kw):
        if filetype is None:
            ...
            if name.endswith(".pdf"):
                ...
                raise Exception(("cannot infer file type " "from extension of file " 'name "{f}"').format(f=filename))
        if filetype in ("pdf", "png", "svg"):
            self._dump_figure(roots, filename, filetype, **kw)
        elif filetype == "pickle":
            self._dump_bdd(roots, filename, **kw)
        else:
            raise Exception('unknown file type "{t}"'.format(t=filetype))

    def _dump_figure(self, roots, filename, filetype, **kw):
        ...
        if filetype == "pdf":
            ...
            raise Exception('Unknown file type of "{f}"'.format(f=filename))
    ...
    def _dump_bdd(self, roots, filename, **kw):
        ...
        d = dict(vars=self.vars, succ=dict(succ))
    ...
    def load(self, filename, levels=True):
        name = filename.lower()
        if name.endswith(".p"):
            return self._load_pickle(filename, levels=levels)
        else:
            raise ValueError('Unknown file type of "{f}"'.format(f=filename))
    ...
    def _load_pickle(self, filename, levels=True):
        ...
        for var, i in items(var2level):
            ...
            if var not in self.vars:
                logger.warning('variable "{var}" added'.format(var=var))
            if levels:
...
def rename(u, bdd, dvars):
    ...
    levels = bdd.vars
    dvars = {levels[var]: levels[dvars.get(var, var)] for var in bdd.vars}
    cache = dict()
...
def _adjacent(i, j, bdd):
    ...
    if abs(i - j) == 1:
        return True
    logger.warning(
        ('level {i} ("{x}") not adjacent to ' 'level {j} ("{y}")').format(
            i=i, j=j, x=bdd.var_at_level(i), y=bdd.var_at_level(j)
        )
    )
    return False
...
def image(trans, source, rename, qvars, bdd, forall=False):
    ...
    rename = {bdd.vars.get(k, k): bdd.vars.get(v, v) for k, v in items(rename)}
    # init
    cache = dict()
    rename_u = rename
    rename_v = None
    ...
    assert not s, s
    return _image(trans, source, rename_u, rename_v, qvars, bdd, forall, cache)
...
def preimage(trans, target, rename, qvars, bdd, forall=False):
    ...
    return _image(trans, target, rename_u, rename_v, qvars, bdd, forall, cache)
...
def reorder(bdd, order=None):
    ...
    len_after = len(bdd)
    logger.info("Reordering changed `BDD` manager size " "from {a} to {b} nodes.".format(a=len_before, b=len_after))
...
def _apply_sifting(bdd):
    ...
    for var in names:
        ...
        m = len(bdd)
        logger.info('{m} nodes for variable "{v}" at level {k}'.format(m=m, v=var, k=k))
    assert m <= n, (m, n)
    ...
    for k in xrange(n):
        for i in xrange(n - 1):
            ...
            if p > q:
                ...
                m += 1
                logger.debug("swap: {p} with {q}, {i}".format(p=p, q=q, i=i))
...
def copy_bdd(u, from_bdd, to_bdd):
    ...
    if from_bdd is to_bdd:
        logger.warning("copying node to same BDD manager")
        return u
    level_map = {from_bdd.level_of_var(var): to_bdd.level_of_var(var) for var in from_bdd.vars if var in to_bdd.vars}
    cache = dict()
