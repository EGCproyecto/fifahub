...
def print_var_levels(bdd):
    ...
    print(("Variable order (starting at level 0):\n" "{levels}").format(levels=levels))


def _raise_runtimerror_about_ref_count(ref_count_lb, name, class_name):
    ...
    raise RuntimeError(
        (
            ...
            "the class {cls_name}."
        ).format(name=name, cls_name=class_name, _ref=ref_count_lb)
    )
