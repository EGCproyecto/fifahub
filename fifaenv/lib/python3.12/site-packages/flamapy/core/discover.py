...
from typing import Any, Optional, Protocol, Type, cast, runtime_checkable

from flamapy.core.config import PLUGIN_PATHS
from flamapy.core.exceptions import ConfigurationNotFound, OperationNotFound, TransformationNotFound
...
class DiscoverMetamodels:
    ...
    def get_plugins_with_operation(self, operation_name: str) -> list[Plugin]:
        return [plugin for plugin in self.plugins if operation_name in self.get_name_operations_by_plugin(plugin.name)]
    ...
    def __transform_to_model_from_file(self, file: str) -> VariabilityModel:
        ...
        t2m_filters = filter(lambda t2m: t2m.get_source_extension() == extension, t2m_transformations)
    ...
    def __search_transformation_way(self, plugin: Plugin, operation_name: str) -> list[tuple[str, str]]:
        """
