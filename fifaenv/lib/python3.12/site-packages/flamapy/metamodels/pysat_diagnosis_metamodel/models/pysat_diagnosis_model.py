...
class DiagnosisModel(PySATModel):
    ...
    def _prepare_assumptions(self, configuration: Configuration = None, test_case: Configuration = None) -> None:
        assumption: List[int] = []
        ...
        if configuration is not None:
            ...
            id_assumption = self._prepare_assumptions_for_configuration(assumption, configuration, id_assumption)

        start_id_test_case = len(assumption)
        if test_case is not None:
            self._prepare_assumptions_for_configuration(assumption, test_case, id_assumption)
    ...
    def _convert_keys_to_features(self, configuration: "Configuration") -> "Configuration":
        new_elements = {
            Feature(key) if isinstance(key, str) else key: value for key, value in configuration.elements.items()
        }
        return Configuration(new_elements)

    def _prepare_assumptions_for_configuration(
        self, assumption: List[int], configuration: Configuration, id_assumption: int
    ) -> int:
