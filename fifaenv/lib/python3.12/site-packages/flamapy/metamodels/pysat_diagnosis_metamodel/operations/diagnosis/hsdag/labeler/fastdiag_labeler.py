...
from .labeler import AbstractHSParameters, IHSLabelable, LabelerType
...
class FastDiagLabeler(FastDiag, IHSLabelable):
    ...
    def get_label(self, parameters: AbstractHSParameters) -> List[List[int]]:
        ...
        if len(parameters.set_c) >= 1 and (
            len(parameters.set_b) == 0 or self.checker.is_consistent(parameters.set_b + neg_c, [])
        ):
    ...
    def identify_new_node_parameters(
        self, param_parent_node: AbstractHSParameters, arc_label: int
    ) -> AbstractHSParameters:
        """
        Identifies the new node's parameters on the basis of the parent node's parameters.
        """
        assert isinstance(param_parent_node, FastDiagParameters), "parameter must be an instance of FastDiagParameters"
