...
from flamapy.metamodels.fm_metamodel.models import Constraint, Feature, FeatureModel
...
def serialize_constraint(ctc: Constraint) -> str:
    ...
    ctc_str = re.sub(rf"\b{ASTOperation.NOT.value}\b", "not", ctc_str)
    ctc_str = re.sub(rf"\b{ASTOperation.AND.value}\b", "&&", ctc_str)
    ctc_str = re.sub(rf"\b{ASTOperation.OR.value}\b", "||", ctc_str)
    ctc_str = re.sub(rf"\b{ASTOperation.IMPLIES.value}\b", "=>", ctc_str)
    ctc_str = re.sub(rf"\b{ASTOperation.EQUIVALENCE.value}\b", "<=>", ctc_str)
    ctc_str = re.sub(rf"\b{ASTOperation.REQUIRES.value}\b", "=>", ctc_str)
    ctc_str = re.sub(rf"\b{ASTOperation.EXCLUDES.value}\b", "=> not", ctc_str)
    return f"[{ctc_str}]"


def attributes_definition(feature_model: FeatureModel) -> str:
    attributes = {
        attribute.get_name(): parse_type_value(attribute.get_default_value())
