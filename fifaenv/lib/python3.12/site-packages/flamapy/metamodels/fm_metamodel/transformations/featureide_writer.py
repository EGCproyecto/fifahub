...
class FeatureIDEWriter(ModelToText):
    ...
    CTC_TYPES = {
        ASTOperation.NOT: FeatureIDEReader.TAG_NOT,
    ...
    def transform(self) -> str:
        ...
        xml_str = ElementTree.tostring(fm_tree, encoding="UTF-8", method="xml", xml_declaration=True)
...
def _to_featureidexml(feature_model: FeatureModel) -> ElementTree.ElementTree:
    ...
    root = ElementTree.SubElement(struct, _tag_element(feature_model.root), _get_attributes(feature_model.root))
...
def _create_tree(parent_element: Element, relations: list[Relation]) -> None:
    for rel in relations:
        for child in rel.children:
            new_elem = ElementTree.SubElement(parent_element, _tag_element(child), _get_attributes(child))
