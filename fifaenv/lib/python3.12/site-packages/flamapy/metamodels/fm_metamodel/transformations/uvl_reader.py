...
class UVLReader(TextToModel):
    ...
    def process_attributes(self, attributes_node: UVLPythonParser.AttributeContext) -> dict[str, Any]:
    ...
    def _check_feature_cardinality(self, feature: Feature, feature_node: UVLPythonParser.FeatureContext) -> None:
    ...
    def _check_attributes(self, feature: Feature, feature_node: UVLPythonParser.FeatureContext) -> None:
    ...
    def process_group(self, group_spec_node: UVLPythonParser.GroupSpecContext) -> list[Feature]:
        list_features = []
    ...
    def process_constraints(self, constraint_node: UVLPythonParser.ConstraintContext) -> Node:
        process = None
    ...
    def process_includes(self, includes_node: UVLPythonParser.IncludesContext) -> list[str]:
    ...
    def process_language_level(self, language_level_node: UVLPythonParser.LanguageLevelContext) -> str:
        ...
        if language_level_node.getChildCount() > 1 and language_level_node.getChild(1).getText() == "*":
            return f"{major_level}.*"

        return major_level

    def process_namespace(self, namespace_node: UVLPythonParser.NamespaceContext) -> str:
        return namespace_node.reference().getText()

    def process_imports(self, imports_node: UVLPythonParser.ImportsContext) -> list[tuple[str, Optional[str]]]:
    ...
    def transform(self) -> FeatureModel:
        ...
        if namespace_node:
            ...
            logging.warning(
                "Namespaces are not meningful for Flama." "This model has the following namespaces: %s ",
                namespace_value,
            )
        ...
        if imports_node:
            ...
            logging.warning(
                "Imports are not yet supported in flama." "This model has the following imports: %s",
                imports_list,
            )
        ...
        if includes_node:
            ...
            logging.warning(
                "Includes are not yet supported in flama." "This model has the following imports: %s",
                includes_list,
            )
