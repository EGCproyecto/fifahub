...
class FMMetrics(Metrics):  # pylint: disable=too-many-instance-attributes
    ...
    def calculate_metamodel_metrics(self, model: VariabilityModel) -> list[dict[str, Any]]:
        ...
        self._constraints_per_features = self.constraints_per_features(self.model, self._features)
        self._feature_ancestors = [
            len(self.get_feature_ancestors(self._features_by_name[f])) for f in self._leaf_features
        ]
    ...
    def features(self) -> dict[str, Any]:
        ...
        result = self.construct_result(name=name, doc=self.features.__doc__, result=_features, size=len(_features))
        return result
    ...
    def solitary_features(self) -> dict[str, Any]:
        ...
        _solitary_features = [
            f.name for f in self._features if not f.is_root() and f.parent is not None and not f.parent.is_group()
        ]
        result = self.construct_result(
            name=name,
    ...
    def grouped_features(self) -> dict[str, Any]:
        ...
        _grouped_features = [
            f.name for f in self._features if not f.is_root() and f.parent is not None and f.parent.is_group()
        ]
        result = self.construct_result(
            name=name,
    ...
    def branching_factor(self) -> dict[str, Any]:
        ...
        result = self.construct_result(name=name, doc=self.branching_factor.__doc__, result=_avg_branching_factor)
        return result
    ...
    def depth_tree(self) -> dict[str, Any]:
        ...
        result = self.construct_result(name=name, doc=self.depth_tree.__doc__, result=_max_depth_tree)
        return result
    ...
    def max_depth_tree(self) -> dict[str, Any]:
        ...
        _max_depth_tree = max(self._feature_ancestors)
        result = self.construct_result(
            name=name, doc=self.max_depth_tree.__doc__, result=_max_depth_tree, parent="Depth of tree", level=1
        )
        return result
    ...
    def strict_complex_constraints(self) -> dict[str, Any]:
        ...
        _strictcomplex_constraints = [str(ctc) for ctc in self.model.get_strictcomplex_constraints()]
        result = self.construct_result(
            name=name,
