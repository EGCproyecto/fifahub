...
class BDDWriter(ModelToText):
    ...
    def transform(self) -> str:
        if not self._roots:
            try:
                self.source_model.bdd.dump(filename=self.path, filetype=self.get_destination_extension())
            except FlamaException:
                roots = [self.source_model.root]
        else:
            roots = [self.source_model.root]
            self.source_model.bdd.dump(filename=self.path, roots=roots, filetype=self.get_destination_extension())
        return ""
