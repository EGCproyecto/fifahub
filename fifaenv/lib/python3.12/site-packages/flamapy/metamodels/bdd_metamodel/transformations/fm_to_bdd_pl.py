...
from flamapy.metamodels.fm_metamodel.models import Constraint, Feature, FeatureModel, Relation
...
class FmToBDD(ModelToModel):
    ...
    def _get_or_formula(self, relation: Relation) -> str:
        assert self.destination_model is not None, "destination_model is None"
        parent = self.destination_model.features_variables[relation.parent.name]
        children = " | ".join(self.destination_model.features_variables[child.name] for child in relation.children)
    ...
    def _get_alternative_formula(self, relation: Relation) -> str:
        ...
        assert self.destination_model is not None, "destination_model is None"
        parent = self.destination_model.features_variables[relation.parent.name]
        children = [self.destination_model.features_variables[child.name] for child in relation.children]
        for child in children:
            children_negatives = set(children) - {child}
            formula.append(f"{child} <=> " f'({" & ".join("!" + ch for ch in children_negatives)} ' f"& {parent})")
    ...
    def _get_mutex_formula(self, relation: Relation) -> str:
        ...
        for child in children:
            ...
            formula.append(f"{child} <=> " f'({" & ".join("!" + cn for cn in children_negatives)} ' f"& {parent})")
        formula_str = " & ".join(f"({f})" for f in formula)
        return f"({parent} <=> " f'!({" | ".join(child for child in children)})) | ({formula_str})'
    ...
    def _get_constraint_formula(self, ctc: Constraint) -> str:
        ...
        constraint_str = re.sub(rf"\b{ASTOperation.XOR.value}\b", BDDModel.LogicConnective.XOR.value, constraint_str)
        constraint_str = re.sub(rf"\b{ASTOperation.NOT.value}\b", BDDModel.LogicConnective.NOT.value, constraint_str)
        constraint_str = re.sub(rf"\b{ASTOperation.AND.value}\b", BDDModel.LogicConnective.AND.value, constraint_str)
        constraint_str = re.sub(rf"\b{ASTOperation.OR.value}\b", BDDModel.LogicConnective.OR.value, constraint_str)
        constraint_str = re.sub(
            rf"\b{ASTOperation.IMPLIES.value}\b", BDDModel.LogicConnective.IMPLIES.value, constraint_str
        )
        constraint_str = re.sub(
            rf"\b{ASTOperation.EQUIVALENCE.value}\b", BDDModel.LogicConnective.EQUIVALENCE.value, constraint_str
        )
        constraint_str = re.sub(
            rf"\b{ASTOperation.REQUIRES.value}\b", BDDModel.LogicConnective.IMPLIES.value, constraint_str
        )
        constraint_str = re.sub(
