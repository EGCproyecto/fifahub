...
def configurations_number(bdd_model: BDDModel, partial_configuration: Optional[Configuration] = None) -> int:
    if partial_configuration is None:
        u_func = bdd_model.root
        n_vars = len(bdd_model.variables_features)
    else:
        values = {bdd_model.features_variables[f]: selected for f, selected in partial_configuration.elements.items()}
