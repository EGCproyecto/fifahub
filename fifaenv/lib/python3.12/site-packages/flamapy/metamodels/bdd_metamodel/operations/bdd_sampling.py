...
class BDDSampling(Sampling):
    ...
    def execute(self, model: VariabilityModel) -> "BDDSampling":
        ...
        self._result = sample(bdd_model, self._sample_size, self._with_replacement, self._partial_configuration)
        return self


def sample(
    model: BDDModel, sample_size: int, with_replacement: bool, partial_configuration: Optional[Configuration]
) -> list[Configuration]:
    configurations = []
...
def random_configuration(bdd_model: BDDModel, p_config: Optional[Configuration] = None) -> Configuration:
    # Initialize the configurations and values for BDD nodes with already known features
    if p_config is None:
        values = {}
    else:
        values = {bdd_model.features_variables[f]: selected for f, selected in p_config.elements.items()}
    ...
    for variable in care_vars:
        ...
        n_vars -= 1
    return Configuration({bdd_model.variables_features[v]: selected for v, selected in values.items()})
