...
class DimacsReader(TextToModel):
    ...
    def transform(self) -> PySATModel:
        with open(self.path, "r", encoding="utf-8") as file:
            ...
            if problem is None:
                raise FlamaException(f"Incorrect Dimacs format of {self.path}. " f"No problem statement.")

            problem_list = problem.split()
            n_clauses = int(problem_list[3])
            if n_clauses != len(clauses_lines):
                raise FlamaException(f"Incorrect Dimacs format of {self.path}. " f"Inconsistent number of clauses.")
