...
class PySATFalseOptionalFeatures(FalseOptionalFeatures):
    ...
    def _get_false_optional_features(self, sat_model: PySATModel) -> list[Any]:
        try:
            ...
            LOGGER.exception(
                "The transformation didn't attach the source model, " "which is required for this operation."
            )

        real_optional_features = [f for f in feature_model.get_features() if not f.is_root() and not f.is_mandatory()]
