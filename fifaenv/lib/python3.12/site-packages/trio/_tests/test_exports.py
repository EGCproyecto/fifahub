    ...
    if tool == "pylint":
        ...
        with mod_cache.open() as cache_file:
            ...
            static_names = no_underscores(
                key for key, value in cache_json["names"].items() if not key.startswith(".") and value["kind"] == "Gdef"
            )
...
def test_static_tool_sees_class_members(
    ...
    for class_name, class_ in module.__dict__.items():
        ...
        if tool == "jedi" and sys.version_info >= (3, 13) and class_ in (trio.Path, trio.WindowsPath, trio.PosixPath):
