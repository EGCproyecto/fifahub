...
def _check_raw_type(
    ...
    if not isinstance(
        ...
        if isinstance(exception, BaseExceptionGroup) and not issubclass(expected_type, BaseExceptionGroup):
...
class AbstractMatcher(ABC, Generic[BaseExcT_co]):
    ...
    def _check_match(self, e: BaseException) -> bool:
        ...
        maybe_specify_type = f" of {_exception_type_name(type(e))}" if isinstance(e, BaseExceptionGroup) else ""
        self._fail_reason = (
            f"Regex pattern {_match_pattern(self.match)!r} did not match {stringified_exception!r}{maybe_specify_type}"
        )
...
class RaisesGroup(AbstractMatcher[BaseExceptionGroup[BaseExcT_co]]):
    ...
    def __init__(
        ...
        *other_exceptions: type[BaseExcT_1] | Matcher[BaseExcT_1] | RaisesGroup[BaseExcT_2],
        ...
        super().__init__(match, check)
        self.expected_exceptions: tuple[type[BaseExcT_co] | Matcher[BaseExcT_co] | RaisesGroup[BaseException], ...] = (
            exception,
            *other_exceptions,
        )
        ...
        for exc in self.expected_exceptions:
            if isinstance(exc, RaisesGroup):
                ...
                raise ValueError(
                    f'Invalid argument "{exc!r}" must be exception type, Matcher, or' " RaisesGroup.",
                )
    ...
    def __repr__(self) -> str:
        parameters = [e.__name__ if isinstance(e, type) else repr(e) for e in self.expected_exceptions]
    ...
    def _check_exceptions(
        ...
        if not failed_expected and results.no_match_for_actual(remaining_actual):
            self._fail_reason = (
                f"{successful_str}Unexpected exception(s): {[actual_exceptions[i] for i in remaining_actual]!r}"
            )
            return False
