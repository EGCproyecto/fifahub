    ...
    def _close(self, exc: BaseException | None) -> BaseException | None:
        if self._cancel_status is None:
            new_exc = RuntimeError(
                f"Cancel scope stack corrupted: attempted to exit {self!r} " "which had already been exited",
            )
...
def unrolled_run(
    ...
    try:
        ...
        warnings.warn(
            RuntimeWarning(
                "Trio guest run got abandoned without properly finishing... " "weird stuff might happen",
            ),
            stacklevel=1,
        )
...
if sys.platform == "win32":
    ...
    if _patchers:
        raise NotImplementedError(
            "unsupported platform or primitives trio depends on are monkey-patched out by " + ", ".join(_patchers),
        )
