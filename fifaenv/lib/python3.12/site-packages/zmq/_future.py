...
class _AsyncSocket(_Async, _zmq.Socket[Future]):
    ...
    def recv_multipart(
        ...
        """
        return self._add_recv_event("recv_multipart", kwargs=dict(flags=flags, copy=copy, track=track))
    ...
    def recv(  # type: ignore
        ...
        """
        return self._add_recv_event("recv", kwargs=dict(flags=flags, copy=copy, track=track))

    def recv_into(self, buf, /, *, nbytes: int = 0, flags: int = 0) -> Awaitable[int]:  # type: ignore
        """Receive a single zmq frame into a pre-allocated buffer.

        Returns a Future, whose result will be the number of bytes received.
        """
        return self._add_recv_event("recv_into", args=(buf,), kwargs=dict(nbytes=nbytes, flags=flags))
    ...
    def _add_recv_event(
        ...
        *,
        ...
        _future_event = _FutureEvent(f, kind, args=args, kwargs=kwargs, msg=None, timer=timer)
    ...
    def _add_send_event(self, kind, msg=None, kwargs=None, future=None):
        ...
        # we add it to the list of futures before we add the timeout as the
        # timeout will remove the future from recv_futures to avoid leaks
        _future_event = _FutureEvent(f, kind, args=(), kwargs=kwargs, msg=msg, timer=timer)
