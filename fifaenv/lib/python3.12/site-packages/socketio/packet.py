...
(CONNECT, DISCONNECT, EVENT, ACK, CONNECT_ERROR, BINARY_EVENT, BINARY_ACK) = (0, 1, 2, 3, 4, 5, 6)
packet_names = ["CONNECT", "DISCONNECT", "EVENT", "ACK", "CONNECT_ERROR", "BINARY_EVENT", "BINARY_ACK"]
...
class Packet:
    ...
    def __init__(self, packet_type=EVENT, data=None, namespace=None, id=None, binary=None, encoded_packet=None):
        ...
        self.id = id
        if self.uses_binary_events and (binary or (binary is None and self._data_is_binary(self.data))):
    ...
    def _reconstruct_binary_internal(self, data, attachments):
        if isinstance(data, list):
            ...
            if data.get("_placeholder") and "num" in data:
                ...
                return {key: self._reconstruct_binary_internal(value, attachments) for key, value in data.items()}
        else:
            return data
    ...
    def _deconstruct_binary_internal(self, data, attachments):
        if isinstance(data, bytes):
            ...
            return [self._deconstruct_binary_internal(item, attachments) for item in data]
        elif isinstance(data, dict):
            return {key: self._deconstruct_binary_internal(value, attachments) for key, value in data.items()}
        else:
            return data
    ...
    def _data_is_binary(self, data):
        ...
        if isinstance(data, bytes):
            ...
            return functools.reduce(lambda a, b: a or b, [self._data_is_binary(item) for item in data], False)
        elif isinstance(data, dict):
            return functools.reduce(lambda a, b: a or b, [self._data_is_binary(item) for item in data.values()], False)
        else:
            return False
