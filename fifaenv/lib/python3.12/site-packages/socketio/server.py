...
class Server(base_server.BaseServer):
    ...
    def send(self, data, to=None, room=None, skip_sid=None, namespace=None, callback=None, ignore_queue=False):
    ...
    def call(self, event, data=None, to=None, sid=None, namespace=None, timeout=60, ignore_queue=False):
    ...
    def disconnect(self, sid, namespace=None, ignore_queue=False):
        ...
        if delete_it:
            ...
            self._send_packet(eio_sid, self.packet_class(packet.DISCONNECT, namespace=namespace))
            self._trigger_event("disconnect", namespace, sid, self.reason.SERVER_DISCONNECT)
    ...
    def _handle_connect(self, eio_sid, namespace, data):
        ...
        if self.always_connect:
            self._send_packet(eio_sid, self.packet_class(packet.CONNECT, {"sid": sid}, namespace=namespace))
        fail_reason = exceptions.ConnectionRefusedError().error_args
        try:
            if data:
                success = self._trigger_event("connect", namespace, sid, self.environ[eio_sid], data)
            else:
                try:
                    success = self._trigger_event("connect", namespace, sid, self.environ[eio_sid])
                except TypeError:
                    success = self._trigger_event("connect", namespace, sid, self.environ[eio_sid], None)
        ...
        if success is False:
            if self.always_connect:
                ...
                self._send_packet(eio_sid, self.packet_class(packet.DISCONNECT, data=fail_reason, namespace=namespace))
            else:
                self._send_packet(
    ...
    def _handle_disconnect(self, eio_sid, namespace, reason=None):
        ...
        self._trigger_event("disconnect", namespace, sid, reason or self.reason.CLIENT_DISCONNECT)
        self.manager.disconnect(sid, namespace, ignore_queue=True)
    ...
    def _handle_event(self, eio_sid, namespace, id, data):
        ...
        if self.async_handlers:
            self.start_background_task(self._handle_event_internal, self, sid, eio_sid, data, namespace, id)
        else:
    ...
    def _handle_event_internal(self, server, sid, eio_sid, data, namespace, id):
        ...
        if r != self.not_handled and id is not None:
            ...
            if r is None:
                ...
                data = [r]
            server._send_packet(eio_sid, self.packet_class(packet.ACK, namespace=namespace, id=id, data=data))
    ...
    def _handle_eio_message(self, eio_sid, data):
        ...
        if eio_sid in self._binary_packet:
            ...
            if pkt.packet_type == packet.CONNECT:
                ...
                self._handle_disconnect(eio_sid, pkt.namespace, self.reason.CLIENT_DISCONNECT)
