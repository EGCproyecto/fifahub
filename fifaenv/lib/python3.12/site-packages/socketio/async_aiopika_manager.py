...
class AsyncAioPikaManager(AsyncPubSubManager):  # pragma: no cover
    ...
    def __init__(self, url="amqp://guest:guest@localhost:5672//", channel="socketio", write_only=False, logger=None):
        if aio_pika is None:
            raise RuntimeError(
                "aio_pika package is not installed " '(Run "pip install aio_pika" in your ' "virtualenv)."
            )
        self.url = url
        ...
        while True:
            try:
                await self.publisher_exchange.publish(
                    aio_pika.Message(body=pickle.dumps(data), delivery_mode=aio_pika.DeliveryMode.PERSISTENT),
                    routing_key="*",
                )
                break
            ...
            while True:
                try:
                    ...
                    self._get_logger().error(
                        "Cannot receive from rabbitmq... " "retrying in {} secs".format(retry_sleep)
                    )
