...
class AsyncManager(BaseManager):
    ...
    async def emit(self, event, data, namespace, room=None, skip_sid=None, callback=None, to=None, **kwargs):
        ...
        if not callback:
            ...
            pkt = self.server.packet_class(packet.EVENT, namespace=namespace, data=[event] + data)
            ...
            for sid, eio_sid in self.get_participants(namespace, room):
                if sid not in skip_sid:
                    for p in eio_pkt:
                        tasks.append(asyncio.create_task(self.server._send_eio_packet(eio_sid, p)))
        else:
            ...
            for sid, eio_sid in self.get_participants(namespace, room):
                if sid not in skip_sid:  # pragma: no branch
                    ...
                    pkt = self.server.packet_class(packet.EVENT, namespace=namespace, data=[event] + data, id=id)
                    tasks.append(asyncio.create_task(self.server._send_packet(eio_sid, pkt)))
