...
class AsyncServer(base_server.BaseServer):
    ...
    async def emit(
        ...
        await self.manager.emit(
            event, data, namespace, room=room, skip_sid=skip_sid, callback=callback, ignore_queue=ignore_queue
        )
    ...
    async def call(self, event, data=None, to=None, sid=None, namespace=None, timeout=60, ignore_queue=False):
        ...
        await self.emit(
            event, data=data, room=to or sid, namespace=namespace, callback=event_callback, ignore_queue=ignore_queue
        )
        try:
    ...
    def session(self, sid, namespace=None):
        ...
        class _session_context_manager:
            ...
            async def __aenter__(self):
                self.session = await self.server.get_session(sid, namespace=self.namespace)
                return self.session

            async def __aexit__(self, *args):
                await self.server.save_session(sid, self.session, namespace=self.namespace)
    ...
    async def disconnect(self, sid, namespace=None, ignore_queue=False):
        ...
        if delete_it:
            ...
            await self._send_packet(eio_sid, self.packet_class(packet.DISCONNECT, namespace=namespace))
            await self._trigger_event("disconnect", namespace, sid, self.reason.SERVER_DISCONNECT)
    ...
    async def _handle_connect(self, eio_sid, namespace, data):
        ...
        if sid is None:
            await self._send_packet(
                eio_sid, self.packet_class(packet.CONNECT_ERROR, data="Unable to connect", namespace=namespace)
            )
            return

        if self.always_connect:
            await self._send_packet(eio_sid, self.packet_class(packet.CONNECT, {"sid": sid}, namespace=namespace))
        fail_reason = exceptions.ConnectionRefusedError().error_args
        try:
            if data:
                success = await self._trigger_event("connect", namespace, sid, self.environ[eio_sid], data)
            else:
                try:
                    success = await self._trigger_event("connect", namespace, sid, self.environ[eio_sid])
                except TypeError:
                    success = await self._trigger_event("connect", namespace, sid, self.environ[eio_sid], None)
        ...
        if success is False:
            if self.always_connect:
                ...
                await self._send_packet(
                    eio_sid, self.packet_class(packet.DISCONNECT, data=fail_reason, namespace=namespace)
                )
            else:
                await self._send_packet(
                    eio_sid, self.packet_class(packet.CONNECT_ERROR, data=fail_reason, namespace=namespace)
                )
    ...
    async def _handle_disconnect(self, eio_sid, namespace, reason=None):
        ...
        await self._trigger_event("disconnect", namespace, sid, reason or self.reason.CLIENT_DISCONNECT)
        await self.manager.disconnect(sid, namespace, ignore_queue=True)
    ...
    async def _handle_event(self, eio_sid, namespace, id, data):
        ...
        if self.async_handlers:
            task = self.start_background_task(self._handle_event_internal, self, sid, eio_sid, data, namespace, id)
    ...
    async def _handle_event_internal(self, server, sid, eio_sid, data, namespace, id):
        ...
        if r != self.not_handled and id is not None:
            ...
            if r is None:
                ...
                data = [r]
            await server._send_packet(eio_sid, self.packet_class(packet.ACK, namespace=namespace, id=id, data=data))
