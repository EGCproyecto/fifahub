...
class RedisManager(PubSubManager):  # pragma: no cover
    ...
    def __init__(
        self, url="redis://localhost:6379/0", channel="socketio", write_only=False, logger=None, redis_options=None
    ):
        if redis is None:
    ...
    def initialize(self):
        ...
        if not monkey_patched:
            raise RuntimeError(
                "Redis requires a monkey patched socket library to work " "with " + self.server.async_mode
            )
    ...
    def _redis_connect(self):
        if not self.redis_url.startswith("redis+sentinel://"):
            ...
            sentinels, service_name, connection_kwargs = parse_redis_sentinel_url(self.redis_url)
