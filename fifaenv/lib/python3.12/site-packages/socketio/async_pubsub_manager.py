...
class AsyncPubSubManager(AsyncManager):
    ...
    async def can_disconnect(self, sid, namespace):
        if self.is_connected(sid, namespace):
            ...
            await self._publish(
                {"method": "disconnect", "sid": sid, "namespace": namespace or "/", "host_id": self.host_id}
            )
    ...
    async def _thread(self):
        while True:
            try:
                async for message in self._listen():  # pragma: no branch
                    ...
                    if data and "method" in data:
                        ...
                        try:
                            ...
                            self.server.logger.exception("Handler error in pubsub listening thread")
                ...
                self.server.logger.exception("Unexpected Error in pubsub " "listening thread")
