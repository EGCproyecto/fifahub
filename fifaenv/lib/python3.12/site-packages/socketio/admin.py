...
class InstrumentedServer:
    ...
    def instrument(self):
        ...
        Socket.handle_post_request = functools.partialmethod(self.__class__._eio_handle_post_request, self)

        # report websocket packets
        Socket.__websocket_handler = Socket._websocket_handler
        Socket._websocket_handler = functools.partialmethod(self.__class__._eio_websocket_handler, self)

        # report connected sockets with each ping
        if self.mode == "development":
            Socket.__send_ping = Socket._send_ping
            Socket._send_ping = functools.partialmethod(self.__class__._eio_send_ping, self)
    ...
    def admin_connect(self, sid, environ, client_auth):
        ...
        def config(sid):
            ...
            self.sio.emit("config", {"supportedFeatures": features}, to=sid, namespace=self.admin_namespace)
            ...
            if self.mode == "development":
                ...
                self.sio.emit("all_sockets", all_sockets, to=sid, namespace=self.admin_namespace)
    ...
    def _emit(self, event, data, namespace, room=None, skip_sid=None, callback=None, **kwargs):
        ret = self.sio.manager.__emit(event, data, namespace, room=room, skip_sid=skip_sid, callback=callback, **kwargs)
        if namespace != self.admin_namespace:
            event_data = [event] + list(data) if isinstance(data, tuple) else [event, data]
        ...
        while not self.stop_stats_event.is_set():
            ...
            self.sio.emit(
                "server_stats",
                {
                    ...
                    "pollingClientsCount": len([s for s in self.sio.eio.sockets.values() if not s.upgraded]),
                    "aggregatedEvents": self.event_buffer.get_and_clear(),
                    "namespaces": [
                        {"name": nsp, "socketsCount": len(self.sio.manager.rooms.get(nsp, {None: []}).get(None, []))}
    ...
    def serialize_socket(self, sid, namespace, eio_sid=None):
        ...
        return {
            ...
            "handshake": {
                ...
                "query": {k: v[0] if len(v) == 1 else v for k, v in parse_qs(environ.get("QUERY_STRING", "")).items()},
                "secure": environ.get("wsgi.url_scheme", "") == "https",
                "url": environ.get("PATH_INFO", ""),
                "issued": tm * 1000,
                "time": datetime.fromtimestamp(tm, timezone.utc).isoformat() if tm else "",
            },
