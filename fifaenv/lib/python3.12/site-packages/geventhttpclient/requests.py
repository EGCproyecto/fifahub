...
class Session(useragent.UserAgent):
    ...
    def request(
        ...
        for param_name, param in dict(cookies=cookies, auth=auth, proxies=proxies, hooks=hooks).items():
            if param is not None:
                raise NotImplementedError(f"{param} is currently unsupported as a keyword argument.")
