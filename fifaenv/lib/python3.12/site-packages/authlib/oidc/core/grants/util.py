...
from ..errors import AccountSelectionRequiredError, ConsentRequiredError, LoginRequiredError
...
def validate_request_prompt(grant, redirect_uri, redirect_fragment=False):
    ...
    prompt = _guess_prompt_value(end_user, prompts, redirect_uri, redirect_fragment=redirect_fragment)
    if prompt:
        grant.prompt = prompt
    return grant
...
def create_response_mode_response(redirect_uri, params, response_mode):
    if response_mode == "form_post":
        ...
        inputs = "".join([f'<input type="hidden" name="{quote_url(k)}" value="{quote_url(v)}"/>' for k, v in params])
...
def _guess_prompt_value(end_user, prompts, redirect_uri, redirect_fragment):
    ...
    if "consent" in prompts:
        if not end_user:
            raise ConsentRequiredError(redirect_uri=redirect_uri, redirect_fragment=redirect_fragment)
        return "consent"
    elif "select_account" in prompts:
        if not end_user:
            raise AccountSelectionRequiredError(redirect_uri=redirect_uri, redirect_fragment=redirect_fragment)
