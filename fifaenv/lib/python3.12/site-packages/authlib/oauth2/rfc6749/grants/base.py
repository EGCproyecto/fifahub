...
class TokenEndpointMixin:
    ...
    def check_token_endpoint(cls, request: OAuth2Request):
        return request.payload.grant_type == cls.GRANT_TYPE and request.method in cls.TOKEN_ENDPOINT_HTTP_METHODS
...
class AuthorizationEndpointMixin:
    ...
    def validate_authorization_redirect_uri(request: OAuth2Request, client):
        if request.payload.redirect_uri:
            ...
            if not redirect_uri:
                raise InvalidRequestError("Missing 'redirect_uri' in request.", state=request.payload.state)
            return redirect_uri
    ...
    def validate_no_multiple_request_parameter(request: OAuth2Request):
        ...
        for param in parameters:
            if len(datalist.get(param, [])) > 1:
                raise InvalidRequestError(f"Multiple '{param}' in request.", state=request.payload.state)
