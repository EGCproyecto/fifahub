...
class AsyncSession(ReversibleProxy[Session]):
    ...
    def get_transaction(self) -> Optional[AsyncSessionTransaction]:
        ...
        if trans is not None:
            return AsyncSessionTransaction._retrieve_proxy_for_target(trans, async_session=self)
        else:
            return None
...
class AsyncSessionTransaction(
    ...
    async def start(self, is_ctxmanager: bool = False) -> AsyncSessionTransaction:
        self.sync_transaction = self._assign_proxied(
            await greenlet_spawn(
                self.session.sync_session.begin_nested if self.nested else self.session.sync_session.begin
            )
        )
        if is_ctxmanager:
