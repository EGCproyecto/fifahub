...
from ...types import BIGINT, BOOLEAN, CHAR, DATE, DOUBLE_PRECISION, FLOAT, INTEGER, NUMERIC, REAL, SMALLINT, TEXT
...
class PGDDLCompiler(compiler.DDLCompiler):
    ...
    def _define_include(self, obj):
        ...
        if not includeclause:
            return ""
        inclusions = [obj.table.c[col] if isinstance(col, str) else col for col in includeclause]
        return " INCLUDE (%s)" % ", ".join([self.preparer.quote(c.name) for c in inclusions])
    ...
    def define_constraint_ondelete_cascade(self, constraint):
        return " ON DELETE %s" % self.preparer.validate_sql_phrase(constraint.ondelete, self._fk_ondelete_pattern)
...
class PGDialect(default.DefaultDialect):
    ...
    def _split_multihost_from_url(self, url: URL) -> Union[
        ...
        if "host" in url.query:
            if isinstance(url.query["host"], (list, tuple)):
                ...
                hosts, ports_str = zip(
                    *[token.split(":") if ":" in token else (token, None) for token in url.query["host"]]
                )
    ...
    def get_multi_pk_constraint(self, connection, schema, filter_names, scope, kind, **kw):
        ...
        return (
            (
                (schema, table_name),
                (pk_constraint(pk_name, cols, comment, opts) if pk_name is not None else default()),
            )
        ...
        query = (
            ...
            .outerjoin(
                ...
                sql.and_(
                    ...
                    pg_catalog.pg_description.c.classoid == sql.func.cast("pg_catalog.pg_class", REGCLASS),
                ),
            )
            .where(self._pg_class_relkind_condition(relkinds))
        )
