    ...
    def __post_init__(self):
        ...
        for field in [
            ...
            if value is not None and not isinstance(value, int):
                raise TypeError(f"{field} must be an integer if" f"provided, got {type(value).__name__}")
...
class VECTOR(types.TypeEngine):
    ...
    def __init__(self, dim=None, storage_format=None):
        ...
        if storage_format is not None and not isinstance(storage_format, VectorStorageFormat):
            raise TypeError("storage_format must be an enum of type VectorStorageFormat")
        self.dim = dim
