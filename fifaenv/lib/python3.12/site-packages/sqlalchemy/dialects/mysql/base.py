...
class MySQLExecutionContext(default.DefaultExecutionContext):
    ...
    def create_server_side_cursor(self) -> DBAPICursor:
        if self.dialect.supports_server_side_cursors:
            return self._dbapi_connection.cursor(self.dialect._sscursor)  # type: ignore[attr-defined]
        else:
...
class MySQLCompiler(compiler.SQLCompiler):
    ...
    def _render_json_extract_from_binary(self, binary: elements.BinaryExpression[Any], operator: Any, **kw: Any) -> str:
    ...
    def visit_json_getitem_op_binary(self, binary: elements.BinaryExpression[Any], operator: Any, **kw: Any) -> str:
    ...
    def visit_on_duplicate_key_update(self, on_duplicate: OnDuplicateClause, **kw: Any) -> str:
        ...
        requires_mysql8_alias = statement.select is None and (self.dialect._requires_alias_for_on_duplicate_key)
        ...
        on_duplicate_update = {
            coercions.expect_as_key(roles.DMLColumnRole, key): value for key, value in on_duplicate.update.items()
        }
        ...
        for column in (col for col in cols if col.key in on_duplicate_update):
            ...
            if coercions._is_literal(val):
                ...
                def replace(element: ExternallyTraversible, **kw: Any) -> Optional[ExternallyTraversible]:
                    ...
                    elif isinstance(element, elements.ColumnClause) and element.table is on_duplicate.inserted_alias:
                        if requires_mysql8_alias:
                            column_literal_clause = f"{_on_dup_alias_name}." f"{self.preparer.quote(element.name)}"
                        else:
                            column_literal_clause = f"VALUES({self.preparer.quote(element.name)})"
    ...
    def visit_concat_op_expression_clauselist(self, clauselist: elements.ClauseList, operator: Any, **kw: Any) -> str:
        return "concat(%s)" % (", ".join(self.process(elem, **kw) for elem in clauselist.clauses))

    def visit_concat_op_binary(self, binary: elements.BinaryExpression[Any], operator: Any, **kw: Any) -> str:
        return "concat(%s, %s)" % (
            self.process(binary.left, **kw),
            self.process(binary.right, **kw),
        )
    ...
    def visit_match_op_binary(self, binary: expression.match, operator: Any, **kw: Any) -> str:
        """
    ...
    def get_from_hint_text(self, table: selectable.FromClause, text: Optional[str]) -> Optional[str]:
        return text
    ...
    def visit_empty_set_expr(self, element_types: List[TypeEngine[Any]], **kw: Any) -> str:
    ...
    def visit_is_distinct_from_binary(self, binary: elements.BinaryExpression[Any], operator: Any, **kw: Any) -> str:
        return "NOT (%s <=> %s)" % (
    ...
    def _mariadb_regexp_flags(self, flags: str, pattern: elements.ColumnElement[Any], **kw: Any) -> str:
        return "CONCAT('(?', %s, ')', %s)" % (
    ...
    def visit_regexp_match_op_binary(self, binary: elements.BinaryExpression[Any], operator: Any, **kw: Any) -> str:
        return self._regexp_match(" REGEXP ", binary, operator, **kw)

    def visit_not_regexp_match_op_binary(self, binary: elements.BinaryExpression[Any], operator: Any, **kw: Any) -> str:
        return self._regexp_match(" NOT REGEXP ", binary, operator, **kw)

    def visit_regexp_replace_op_binary(self, binary: elements.BinaryExpression[Any], operator: Any, **kw: Any) -> str:
        assert binary.modifiers is not None
        flags = binary.modifiers["flags"]
        if flags is None:
...
class MySQLDDLCompiler(compiler.DDLCompiler):
    ...
    def define_constraint_match(self, constraint: sa_schema.ForeignKeyConstraint) -> str:
...
class MySQLTypeCompiler(compiler.GenericTypeCompiler):
    ...
    def _extend_string(self, type_: _StringType, defaults: Dict[str, Any], spec: str) -> str:
    ...
    def _visit_enumerated_values(self, name: str, type_: _StringType, enumerated_values: Sequence[str]) -> str:
        quoted_enums = []
...
class MySQLDialect(default.DefaultDialect):
    ...
    def _set_mariadb(self, is_mariadb: Optional[bool], server_version_info: Tuple[int, ...]) -> None:
    ...
    def is_disconnect(
        ...
        if isinstance(
            ...
            return True
        elif isinstance(e, (self.dbapi.InterfaceError, self.dbapi.InternalError)):  # type: ignore  # noqa: E501
    ...
    def _compat_first(self, rp: CursorResult[Any], charset: Optional[str] = None) -> Optional[_DecodingRow]:
        """Proxy a result row to smooth over MySQL-Python driver
        inconsistencies."""
    ...
    def _is_mariadb_102(self) -> bool:
        return self.is_mariadb and self._mariadb_normalized_version_info > (  # type:ignore[operator]
            10,
            2,
        )
    ...
    def _correct_for_mysql_bugs_88718_96365(
        ...
        schema_by_table_by_column: DefaultDict[str, DefaultDict[str, List[str]]] = DefaultDict(
        ...
        if schema_by_table_by_column:
            ...
            correct_for_wrong_fk_case: CursorResult[Tuple[str, str, str]] = connection.execute(select)
            ...
            for fkey in fkeys:
                ...
                fkey["referred_columns"] = [rec_b[col.lower()] for col in fkey["referred_columns"]]
    ...
    def get_check_constraints(
        ...
        **kw: Any,
        ...
        cks: List[ReflectedCheckConstraint] = [
            {"name": spec["name"], "sqltext": spec["sqltext"]} for spec in parsed_state.ck_constraints
        ]
    ...
    def _setup_parser(
        ...
        **kw: Any,
        ...
        if parser._check_view(sql):
            ...
            sql = parser._describe_to_create(table_name, columns)  # type: ignore[arg-type]
        return parser.parse(sql, charset)

    def _fetch_setting(self, connection: Connection, setting_name: str) -> Optional[str]:
        charset = self._connection_charset
...
class _DecodingRow:
    ...
    def __init__(self, rowproxy: Row[Any], charset: Optional[str]):
        ...
        self.charset = self._encoding_compat.get(charset, charset) if charset is not None else None
