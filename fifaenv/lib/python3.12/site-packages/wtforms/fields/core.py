        ...
        if validators is not None:
            for validator in validators:
                if not callable(validator):
                    raise TypeError(f"{validator} is not a valid validator because it is not " "callable")

                if inspect.isclass(validator):
                    raise TypeError(
                        f"{validator} is not a valid validator because it is a class, " "it should be an instance"
                    )
...
class UnboundField:
    ...
    def __repr__(self):
        return "<UnboundField(" f"{self.field_class.__name__}, {self.args!r}, {self.kwargs!r}" ")>"
...
class Flags:
    ...
    def __repr__(self):
        flags = (f"{name}={getattr(self, name)}" for name in dir(self) if not name.startswith("_"))
        flags = ", ".join(flags)
