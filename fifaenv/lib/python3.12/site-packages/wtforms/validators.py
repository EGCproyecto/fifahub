...
class EqualTo:
    ...
    def __call__(self, form, field):
        try:
            ...
            raise ValidationError(field.gettext("Invalid field name '%s'.") % self.fieldname) from exc
...
class Email:
    ...
    def __call__(self, form, field):
        try:
            ...
            raise Exception("Install 'email_validator' for email validation support.") from exc
...
class URL(Regexp):
    ...
    def __init__(self, require_tld=True, allow_ip=True, message=None):
        regex = r"^[a-z]+://" r"(?P<host>[^\/\?:]+)" r"(?P<port>:[0-9]+)?" r"(?P<path>\/.*?)?" r"(?P<query>\?.*)?$"
...
class HostnameValidation:
    ...
    def __call__(self, hostname):
        if self.allow_ip and (IPAddress.check_ipv4(hostname) or IPAddress.check_ipv6(hostname)):
            return True
...
class Disabled:
    ...
    def __call__(self, form, field):
        if field.raw_data is not None:
            raise ValidationError(field.gettext("This field is disabled and cannot have a value."))
