...
class DocumentMeta(type):
    ...
    def __new__(cls, name: str, bases: Tuple[type, ...], attrs: Dict[str, Any]) -> "DocumentMeta":
        ...
        for name in fields:
            ...
            if name in attrs:
                ...
                if isinstance(attr_value, dict):
                    ...
                    default_value = attrs[name].get("default") or attrs[name].get("default_factory")
                    if default_value:
...
class Mapped(Generic[_FieldType]):
    ...
    if TYPE_CHECKING:
        ...
        def __get__(self, instance: Optional[object], owner: Any) -> Union[InstrumentedField, _FieldType]: ...
...
class DocumentBase(ObjectBase):
    ...
    def _get_index(self, index: Optional[str] = None, required: bool = True) -> Optional[str]:
        if index is None:
    ...
    def __repr__(self) -> str:
        return "{}({})".format(
            ...
            ", ".join(f"{key}={getattr(self.meta, key)!r}" for key in ("index", "id") if key in self.meta),
        )
