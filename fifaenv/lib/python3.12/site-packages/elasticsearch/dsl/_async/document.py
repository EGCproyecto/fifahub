...
class AsyncIndexMeta(DocumentMeta):
    ...
    def __new__(cls, name: str, bases: Tuple[type, ...], attrs: Dict[str, Any]) -> "AsyncIndexMeta":
    ...
    @classmethod
    def construct_index(cls, opts: Dict[str, Any], bases: Tuple[type, ...]) -> AsyncIndex:
        if opts is None:
            for b in bases:
                if hasattr(b, "_index"):
                    return b._index
...
class AsyncDocument(DocumentBase, metaclass=AsyncIndexMeta):
    ...
    @classmethod
    def _get_connection(cls, using: Optional[AsyncUsingType] = None) -> "AsyncElasticsearch":
        return get_connection(cls._get_using(using))

    @classmethod
    async def init(cls, index: Optional[str] = None, using: Optional[AsyncUsingType] = None) -> None:
        """
    ...
    @classmethod
    def search(cls, using: Optional[AsyncUsingType] = None, index: Optional[str] = None) -> AsyncSearch[Self]:
        """
        Create an :class:`~elasticsearch.dsl.Search` instance that will search
        over this ``Document``.
        """
        return AsyncSearch(using=cls._get_using(using), index=cls._default_index(index), doc_type=[cls])
    ...
    async def mget(
        ...
        **kwargs: Any,
        ...
        es = cls._get_connection(using)
        body = {"docs": [doc if isinstance(doc, collections.abc.Mapping) else {"_id": doc} for doc in docs]}
    ...
    async def update(
        ...
        **fields: Any,
        ...
        meta = await self._get_connection(using).update(index=i, body=body, refresh=refresh, **doc_meta)
        ...
        **kwargs: Any,
        ...
        class Generate:
            ...
            async def __anext__(self) -> Dict[str, Any]:
                doc: Optional[Union[AsyncDocument, Dict[str, Any]]] = await self.doc_iterator.__anext__()

                if isinstance(doc, dict):
                    action = doc
                    doc = None
                    if "_source" in action and isinstance(action["_source"], AsyncDocument):
                        doc = action["_source"]
                        if validate:  # pragma: no cover
                            doc.full_clean()
                        action["_source"] = doc.to_dict(include_meta=False, skip_empty=skip_empty)
