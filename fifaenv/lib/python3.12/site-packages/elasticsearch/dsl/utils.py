...
class AttrList(Generic[_ValT]):
    def __init__(self, l: List[_ValT], obj_wrapper: Optional[Callable[[_ValT], Any]] = None):
    ...
    def __setstate__(self, state: Tuple[List[_ValT], Optional[Callable[[_ValT], Any]]]) -> None:
...
class AttrDict(Generic[_ValT]):
    ...
    def __setattr__(self, name: str, value: _ValT) -> None:
        ...
        if (name in self._d_ or not hasattr(self.__class__, name)) and name != "__orig_class__":
    ...
    def to_dict(self, recursive: bool = False) -> Dict[str, _ValT]:
        return cast(Dict[str, _ValT], _recursive_to_dict(self._d_) if recursive else self._d_)
...
class DslBase(metaclass=DslMeta):
    ...
    def get_dsl_class(cls: Type[Self], name: str, default: Optional[str] = None) -> Type[Self]:
        try:
            ...
            raise UnknownDslObject(f"DSL class `{name}` does not exist in {cls._type_name}.")
            ...
            if "type" in pinfo:
                ...
                if pinfo.get("multi") and pinfo.get("hash"):
                    if not isinstance(value, (tuple, list)):
                        value = (value,)
                    value = list({k: shortcut(v) for (k, v) in obj.items()} for obj in value)
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        for pname, value in self._params.items():
            ...
            if pinfo and "type" in pinfo:
                ...
                # list of dict(name -> DslBase)
                if pinfo.get("multi") and pinfo.get("hash"):
                    value = list({k: v.to_dict() for k, v in obj.items()} for obj in value)
