...
class Request(Generic[_R]):
    def __init__(
        ...
        doc_type: Optional[Union[type, str, List[Union[type, str]], Dict[str, Union[type, str]]]] = None,
    ...
    def _resolve_nested(self, hit: AttrDict[Any], parent_class: Optional[type] = None) -> Type[_R]:
        doc_class = Hit
        ...
        if parent_class is not None and hasattr(parent_class, "_index"):
            nested_field = cast(Optional["Object"], parent_class._index.resolve_field(nested_path_str))
        else:
            nested_field = cast(Optional["Object"], self._resolve_field(nested_path_str))
    ...
    def _get_result(self, hit: AttrDict[Any], parent_class: Optional[type] = None) -> _R:
        doc_class: Any = Hit
        dt = hit.get("_type")
        ...
        for t in hit.get("inner_hits", ()):
            hit["inner_hits"][t] = Response[_R](self, hit["inner_hits"][t], doc_class=doc_class)

        callback = getattr(doc_class, "from_es", doc_class)
        return cast(_R, callback(hit))

    def doc_type(self, *doc_type: Union[type, str], **kwargs: Callable[[AttrDict[Any]], Any]) -> Self:
        """
...
class SearchBase(Request[_R]):
    ...
    def update_from_dict(self, d: Dict[str, Any]) -> Self:
        ...
        if aggs:
            self.aggs._params = {"aggs": {name: A(value) for (name, value) in aggs.items()}}
        if "knn" in d:
            self._knn = d.pop("knn")
    ...
    def knn(
        ...
        if query_vector is not None and query_vector_builder is not None:
            raise ValueError("only one of query_vector and query_vector_builder must be given")
    ...
    def sort(self, *keys: Union[str, "InstrumentedField", Dict[str, Dict[str, str]]]) -> Self:
        """
    ...
    def highlight(self, *fields: Union[str, "InstrumentedField"], **kwargs: Any) -> Self:
        """
