...
class Lexer:
    ...
    def __init__(self, debug: bool = False) -> None:
        self.reserved = getattr(self, "reserved", dict())
        self.delimiters = getattr(self, "delimiters", list())
        self.operators = getattr(self, "operators", list())
        self.misc = getattr(self, "misc", list())
        self.logger = getattr(self, "logger", logger)
        self.tokens = self.delimiters + self.operators + self.misc + sorted(set(self.reserved.values()))
        self.build(debug=debug)

    def t_error(self, t) -> _ty.NoReturn:
        raise RuntimeError(f'Illegal character "{t.value[0]}"')

    def build(self, debug: bool = False, debuglog=None, **kwargs) -> None:
        """Create a lexer."""
...
class Parser:
    ...
    def __init__(self, nodes=None, lexer=None) -> None:
        self.tabmodule = getattr(self, "tabmodule", None)
        self.start = getattr(self, "start", "expr")
        # low to high
        self.precedence = getattr(self, "precedence", tuple())
        self.nodes = getattr(self, "nodes", _ast)
        self.logger = getattr(self, "logger", logger)
        ...
        if lexer is not None:
            ...
            raise ValueError("pass argument `lexer` to " "`Parser.__init__()`")
        self.tokens = self._lexer.tokens
        self.parser = None

    def build(
        self,
        tabmodule: str | None = None,
        outputdir: str = "",
    ...
    def parse(self, formula: str, debuglog=None) -> _ty.Any:
        ...
        if self.parser is None:
            self.build()
        root = self.parser.parse(input=formula, lexer=self._lexer.lexer, debug=debuglog)
        self._clear_lr_stack()
        if root is not None:
            return root
        raise RuntimeError(f"failed to parse:\n\t{formula}")

    def _clear_lr_stack(self) -> None:
        ...
        """
        has_lr_stack = (
            self.parser is not None and hasattr(self.parser, "statestack") and hasattr(self.parser, "symstack")
        )
        if not has_lr_stack:
            return
        self.parser.restart()

    def p_error(self, p) -> _ty.NoReturn:
        s = list()
        while True:
            ...
            s.append(tok.value)
        s = " ".join(s)
        raise RuntimeError(f'Syntax error at "{p.value}"\n' f"remaining input:\n{s}\n")


def rewrite_tables(parser_class: type[Parser], tabmodule: str, outputdir: str) -> None:
    ...
    if outputdir is None:
        raise ValueError("`outputdir` must be `str`.")
    *_, table = tabmodule.rpartition(".")
    ...
    with open(path, "w") as debuglog_file:
        ...
        parser.build(write_tables=True, outputdir=outputdir, tabmodule=table, debug=True, debuglog=debuglog)
