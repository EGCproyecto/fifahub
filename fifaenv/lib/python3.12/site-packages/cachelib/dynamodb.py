...
class DynamoDbCache(BaseCache):
    ...
    def _set(
        ...
        if not overwrite:
            ...
            cond = self._attr(self._key_field).not_exists() | self._attr(self._expiration_time_field).lte(
                int(now.timestamp())
            )
    ...
    def has(self, key: str) -> bool:
        return self._get_item(self.key_prefix + key, [self._expiration_time_field]) is not None

    def clear(self) -> bool:
        paginator = self._dynamo.meta.client.get_paginator("scan")
        pages = paginator.paginate(TableName=self._table_name, ProjectionExpression=self._key_field)
