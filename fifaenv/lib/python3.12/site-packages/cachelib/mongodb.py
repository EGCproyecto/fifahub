...
class MongoDbCache(BaseCache):
    ...
    def __init__(
        ...
        **kwargs: _t.Any,
        ...
        all_keys = {subkey[0] for value in index_info.values() for subkey in value["key"]}
    ...
    def set_many(self, mapping: _t.Dict[str, _t.Any], timeout: _t.Optional[int] = None) -> _t.List[_t.Any]:
        ...
        if result.bulk_api_result["nUpserted"] != len(keys):
            query = self.client.find({"id": {"$in": [self.key_prefix + key for key in keys]}})
            keys = []
            for item in query:
                keys.append(item["id"])
        return keys
    ...
    def delete_many(self, *keys: str) -> _t.List[_t.Any]:
        ...
        if result.deleted_count != len(keys):
            existing_keys = [item["id"][len(self.key_prefix) :] for item in self.client.find(filter)]
