...
class LSU:
    ...
    def _get_model_cost(self, formula, model):
        ...
        for i, cl in enumerate(formula.soft):
            cost += (
                formula.wght[i]
                if all(l not in model_set for l in filter(lambda l: abs(l) <= self.formula.nv, cl))
                else 0
            )
...
class LSUPlus(LSU, object):
    ...
    def __init__(self, formula, solver, expect_interrupt=False, verbose=0):
        ...
        assert (
            solver in SolverNames.gluecard3 or solver in SolverNames.gluecard4 or solver in SolverNames.minicard
        ), "{0} does not support native cardinality constraints".format(solver)

        super(LSUPlus, self).__init__(formula, solver=solver, expect_interrupt=expect_interrupt, verbose=verbose)
...
if __name__ == "__main__":
    ...
    if files:
        ...
        if re.search("cnf(\.(gz|bz2|lzma|xz))?$", files[0]):
            ...
            lsu = LSU(formula, solver=solver, expect_interrupt=(timeout != None), verbose=verbose)

        # reading WCNF+
        elif re.search("\.wcnf[p,+](\.(gz|bz2|lzma|xz))?$", files[0]):
            formula = WCNFPlus(from_file=files[0])
            lsu = LSUPlus(formula, solver=solver, expect_interrupt=(timeout != None), verbose=verbose)
