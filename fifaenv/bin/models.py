...
def enumerate_models(formula, to_enum, solver, warm=False):
    ...
    with Solver(name=solver, bootstrap_with=formula.clauses, use_timer=True, warm_start=warm) as s:
        # adding native cardinality constraints if needed
        if formula.atmosts:
            assert (
                solver in SolverNames.minicard or solver in SolverNames.gluecard3 or solver in SolverNames.gluecard4
...
def parse_options():
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], "e:h:s:w", ["enum=", "help", "solver=", "warm"])
