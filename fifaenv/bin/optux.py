...
class OptUx(object):
    ...
    def __init__(
        ...
        if not unsorted:
            ...
            self.hitman = Hitman(
                bootstrap_with=to_hit, weights=self.weights, solver=solver, htype="lbx", mcs_usecld=dcalls
            )
        else:
...
def parse_options():
    ...
    return adapt, cover, dcalls, exhaust, minz, trim, to_enum, solver, puresat, unsorted, verbose, args
...
def usage():
    ...
    print(
        "                                  Available values: g3, g4, lgl, mcb, mcm, mpl, m22, mc, mgh (default = m22)"
    )
...
if __name__ == "__main__":
    adapt, cover, dcalls, exhaust, minz, trim, to_enum, solver, puresat, unsorted, verbose, files = parse_options()
