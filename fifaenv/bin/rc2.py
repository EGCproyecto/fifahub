#!/home/sergio/fifahub/fifaenv/bin/python3.12
# -*- coding:utf-8 -*-
##
## rc2.py
##
...
class RC2(object):
    ...
    def add_clause(self, clause, weight=None):
        ...
        *enumerating* MaxSAT solutions of the formula. This way,
        ...
        cl = list(
            map(
                lambda l: self._map_extlit(l),
                clause if not len(clause) == 2 or not type(clause[0]) in (list, tuple, set) else clause[0],
            )
        )
    ...
    def compute_(self):
        ...
        while not self.oracle.solve(assumptions=self.sels + self.sums):
            ...
            if self.verbose > 1:
                print(
                    "c cost: {0}; core sz: {1}; soft sz: {2}".format(
                        self.cost, len(self.core), len(self.sels) + len(self.sums)
                    )
                )
    ...
    def adapt_am1(self):
        ...
        if self.verbose > 1 and nof_am1:
            print(
                "c am1s found: {0}; avgsz: {1:.1f}; cost: {2}".format(nof_am1, sum(len_am1) / float(nof_am1), self.cost)
            )
...
class RC2Stratified(RC2, object):
    ...
    def __init__(
        ...
        super(RC2Stratified, self).__init__(
            formula, solver=solver, adapt=adapt, exhaust=exhaust, incr=incr, minz=minz, trim=trim, verbose=verbose
        )
...
def parse_options():
    ...
    return adapt, blo, block, cmode, to_enum, exhaust, incr, minz, solver, trim, verbose, vnew, args
...
if __name__ == "__main__":
    adapt, blo, block, cmode, to_enum, exhaust, incr, minz, solver, trim, verbose, vnew, files = parse_options()
