...
class Counter(object):
    ...
    *confidence parameter* ``delta``.
    ...
    def __init__(self, formula=None, seed=1, epsilon=0.8, delta=0.2, verbose=0):
        ...
        self.counter = pyapproxmc.Counter(verbosity=verbose, seed=seed, epsilon=epsilon, delta=delta)
...
def parse_options():
    ...
    try:
        opts, args = getopt.getopt(
            sys.argv[1:], "d:e:hp:s:v:", ["delta=", "epsilon=", "help", "projection=", "seed=", "verbose="]
        )
...
if __name__ == "__main__":
    ...
    if files and re.search("\.cnf(\.(gz|bz2|lzma|xz))?$", files[0]):
        ...
        # creating the counter object
        with Counter(formula, seed=seed, epsilon=epsilon, delta=delta, verbose=verbose) as counter:
