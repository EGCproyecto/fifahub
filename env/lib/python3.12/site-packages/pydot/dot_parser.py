...
class HTML(Token):
    ...
    def parseImpl(self, instring: str, loc: int, do_actions: bool = True) -> tuple[int, str]:
        open_loc = loc
        ...
        raise ParseException(
            instring,
            loc,
            "HTML: expected '>' to match '<' " + f"on line {lineno(open_loc, instring)}",
            self,
        )
...
class GraphParser:
    ...
    ID = (
        concat_string("concat") | double_quoted | identifier("ident") | HTML().set_results_name("html")
    ...
    node_id = DelimitedList(Group(ID("id_part")), delim=":", min=1, max=3, combine=False).set_parse_action(push_node_id)

    a_list = OneOrMore(ID + Optional(equals + righthand_id) + Optional(comma.suppress()))
    attr_list = OneOrMore(lbrack.suppress() + Optional(a_list) + rbrack.suppress())
    node_stmt = node_id("name") + Optional(attr_list("attr_l")) + Optional(semi.suppress())

    default_type = graph_ | node_ | edge_
    default_stmt = default_type("dtype") + attr_list("attr_l")

    stmt_list = Forward()
    graph_stmt = Group(lbrace.suppress() + Optional(stmt_list) + rbrace.suppress() + Optional(semi.suppress()))
    ...
    edge_stmt = DelimitedList(edge_point, delim=edgeop, min=2)("endpoints") + Optional(attr_list("attr_l"))
    ...
    parser = OneOrMore(
        Group(Optional(strict_("strict")) + graph_type("gtype") + Optional(ID("id")) + graph_stmt("contents"))
