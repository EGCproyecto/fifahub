...
def schema(type: Any, *, schema_hook: Optional[Callable[[type], dict[str, Any]]] = None) -> dict[str, Any]:
...
class _SchemaGenerator:
    ...
    def to_schema(self, t: mi.Type, check_ref: bool = True) -> dict[str, Any]:
        ...
        if isinstance(t, (mi.AnyType, mi.RawType)):
            ...
            if len(structs) >= 2:
                mapping = {k: self.ref_template.format(name=self.name_map[v.cls]) for k, v in structs.items()}
                struct_schema = {
