...
def test_display_arg_handling_node_alpha(param_value, expected):
    ...
    assert all(canvas.get_children()[0].get_fc()[:, 3] == expected)  # Extract just the alpha from the node colors
    plt.close()
...
def test_display_line_collection():
    ...
    nx.set_edge_attributes(G, {(u, v): "-|>" if (u + v) % 2 else "-" for u, v in G.edges()}, "arrowstyle")
    canvas = plt.figure().add_subplot(111)
...
def test_display_shortest_path():
    ...
    nx.set_edge_attributes(
        ...
        {
            (u, v): {
                "color": ("red" if (u, v) in path_edges or tuple(reversed((u, v))) in path_edges else "black"),
...
def test_display_edge_width(param_value, expected, graph_type):
    ...
    if G.is_directed():
        ...
        widths = list(
            [l for l in canvas.collections if isinstance(l, mpl.collections.LineCollection)][0].get_linewidths()
        )
...
def test_display_edge_style(param_value, expected, graph_type):
    ...
    if G.is_directed():
        ...
        styles = [
            ...
            for s in [l for l in canvas.collections if isinstance(l, mpl.collections.LineCollection)][
                0
            ].get_linestyles()
        ]
...
def test_display_multigraph_non_integer_keys():
    ...
    nx.set_edge_attributes(G, {e: f"arc3,rad={0.2 * int(e[2])}" for e in G.edges(keys=True)}, "curvature")
    canvas = plt.figure().add_subplot(111)
    nx.display(G, canvas=canvas)
    rads = [f.get_connectionstyle().rad for f in canvas.get_children() if isinstance(f, mpl.patches.FancyArrowPatch)]
...
def test_display_arrow_size():
    ...
    nx.display(G, canvas=ax)
    assert [9, 25, 49] == [
        f.get_mutation_scale() for f in ax.get_children() if isinstance(f, mpl.patches.FancyArrowPatch)
    ]
    plt.close()
...
def test_individualized_edge_attributes(subplots):
    ...
    for e, a in zip(G.edges(), arrows):
        ...
        expected = mpl.patches.ArrowStyle.BracketB if sum(e) % 2 else mpl.patches.ArrowStyle.CurveFilledB
...
def test_edge_label_bar_connectionstyle(subplots):
    ...
    arc_lbl = nx.draw_networkx_edge_labels(G, pos, edge_labels={edge: "edge"}, connectionstyle=style_arc)
    # This would fail prior to gh-7739
    bar_lbl = nx.draw_networkx_edge_labels(G, pos, edge_labels={edge: "edge"}, connectionstyle=style_bar)
