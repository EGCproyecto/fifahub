    ...
    **kwargs,
    ...
    for kwarg in kwargs:
        if kwarg not in defaults:
            raise nx.NetworkXError(f"Unrecongized visualization keyword argument: {kwarg}")
    ...
    def get_edge_attr(e, attr):
        ...
        if attr is not None and nx.get_edge_attributes(edge_subgraph, attr) == {} and attr in kwargs.values():
            return attr
    ...
    def get_node_attr(n, attr, use_edge_subgraph=True):
        ...
        if attr is not None and nx.get_node_attributes(subgraph, attr) == {} and attr in kwargs.values():
            return attr
    ...
    def self_loop(edge_index, node_size):
        def self_loop_connection(posA, posB, *args, **kwargs):
            if not np.all(posA == posB):
                raise nx.NetworkXError("`self_loop` connection style method" "is only to be used for self-loops")
    ...
    if callable(pos):
        ...
        nx.set_node_attributes(node_subgraph, nx.spring_layout(node_subgraph), default_display_pos_attr)
        pos = default_display_pos_attr
        kwargs["node_pos"] = default_display_pos_attr
    ...
    if len(visible_nodes) > 0:
        ...
        for shape in Counter(nx.get_node_attributes(node_subgraph, node_shape, defaults["node_shape"]).values()):
...
def draw_networkx_edge_labels(
    ...
    class CurvedArrowText(mpl.text.Text):
        ...
        def _get_arrow_path_disp(self, arrow):
            ...
            if arrow._posA_posB is None:
                raise ValueError("Can only draw labels for fancy arrows with " "posA and posB inputs, not custom path")
            posA = arrow._convert_xy_units(arrow._posA_posB[0])
            posB = arrow._convert_xy_units(arrow._posA_posB[1])
            (posA, posB) = trans_data.transform((posA, posB))
            _path = arrow.get_connectionstyle()(
        ...
        def _update_text_pos_angle(self, arrow):
            ...
            tt = 1 - t
            path_disp = self._get_arrow_path_disp(arrow)
            is_bar_style = isinstance(arrow.get_connectionstyle(), mpl.patches.ConnectionStyle.Bar)
            # 1. Calculate x and y
            if is_bar_style:
    ...
    if isinstance(connectionstyle, str):
        ...
        raise nx.NetworkXError("draw_networkx_edges arg `connectionstyle` must be" "string or iterable of strings")
