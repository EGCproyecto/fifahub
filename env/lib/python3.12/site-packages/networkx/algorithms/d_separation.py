...
def is_minimal_d_separator(G, x, y, z, *, included=None, restricted=None):
    ...
    try:
        ...
        raise nx.NodeNotFound("One of x, y, z, included or restricted is not a node or set of nodes in G")

    if not included <= z:
        raise nx.NetworkXError(f"Included nodes {included} must be in proposed separating set z {x}")
    if not z <= restricted:
        raise nx.NetworkXError(f"Separating set {z} must be contained in restricted set {restricted}")
