...
def _neighborhood_aggregate_directed(G, node, node_labels, edge_attr=None):
    ...
    return node_labels[node] + "".join(sorted(successor_labels)) + "".join(sorted(predecessor_labels))
...
def weisfeiler_lehman_subgraph_hashes(
    ...
    if G.is_directed():
        _neighborhood_aggregate = _neighborhood_aggregate_directed
        warnings.warn(
            "The hashes produced for directed graphs changed in v3.5" " due to a bugfix (see documentation).",
            UserWarning,
            stacklevel=2,
        )
    else:
