...
class APIDocsView(MethodView):
    ...
    def get(self):
        ...
        data = {"specs": specs, "urls": urls, "title": self.config.get("title", "Flasgger")}
        if request.args.get("json"):
            ...
            data["favicon"] = self.config.get("favicon", url_for("flasgger.static", filename="favicon-32x32.png"))
            data["swagger_ui_bundle_js"] = self.config.get(
                "swagger_ui_bundle_js", url_for("flasgger.static", filename="swagger-ui-bundle.js")
            )
            ...
            data["jquery_js"] = self.config.get("jquery_js", url_for("flasgger.static", filename="lib/jquery.min.js"))
...
class Swagger(object):
    ...
    SCHEMA_LOCATIONS = {"query": "args", "header": "headers", "formData": "form", "body": "json", "path": "path"}
    ...
    def get_apispecs(self, endpoint="apispec_1"):
        ...
        for _spec in self.config["specs"]:
            if _spec["endpoint"] == endpoint:
                ...
                break
        if not spec:
            raise RuntimeError("Can`t find specs by endpoint {}," " check your flasger`s config".format(endpoint))
        ...
        for rule, verbs in specs:
            ...
            for verb, swag in verbs:
                ...
                defs += extract_definitions(
                    defs, endpoint=rule.endpoint, verb=verb, prefix_ids=prefix_ids, openapi_version=openapi_version
                )
                ...
                defs += extract_definitions(
                    params, endpoint=rule.endpoint, verb=verb, prefix_ids=prefix_ids, openapi_version=openapi_version
                )
    ...
    def register_views(self, app):
        ...
        if self.config.get("swagger_ui", True):
            ...
            blueprint = Blueprint(
                ...
                template_folder=self.config.get("template_folder", "ui{0}/templates".format(uiversion)),
                static_folder=self.config.get("static_folder", "ui{0}/static".format(uiversion)),
            ...
            blueprint.add_url_rule("/apidocs/index.html", view_func=lambda: redirect(url_for("flasgger.apidocs")))
        else:
        ...
        for spec in self.config["specs"]:
            ...
            blueprint.add_url_rule(
                ...
                spec["endpoint"],
                view_func=wrap_view(
                    APISpecsView.as_view(spec["endpoint"], loader=partial(self.get_apispecs, endpoint=spec["endpoint"]))
                ),
            )
