...
def get_repeating_interval_resolution(isointervalstr, intervaldelimiter="/", datetimedelimiter="T"):
...
def parse_interval(
    ...
    if intervaldelimitercount == 0:
        raise ISOFormatError(
            'Interval delimiter "{0}" is not in interval ' 'string "{1}".'.format(intervaldelimiter, isointervalstr)
        )
...
def _parse_interval(isointervalstr, builder, intervaldelimiter="/", datetimedelimiter="T"):
    ...
    if firstpart[0] == "P":
        ...
        if secondpart.find(datetimedelimiter) != -1:
            # <end> is a datetime
            endtuple = parse_datetime(secondpart, delimiter=datetimedelimiter, builder=TupleBuilder)
        else:
    ...
    if secondpart[0] == "P":
        ...
        if firstpart.find(datetimedelimiter) != -1:
            # <start> is a datetime
            starttuple = parse_datetime(firstpart, delimiter=datetimedelimiter, builder=TupleBuilder)
        else:
