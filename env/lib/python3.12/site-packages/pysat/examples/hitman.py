    ...
    def init(self, bootstrap_with, weights=None, subject_to=[]):
        ...
        for cl in subject_to:
            if not len(cl) == 2 or not type(cl[0]) in (list, tuple, set):
                ...
                formula.append(
                    [list(map(lambda a: self.idpool.id(a.obj) * (2 * a.sign - 1), cl[0])), cl[1]], is_atmost=True
                )

        # soft clauses
        for obj_id in six.iterkeys(self.idpool.id2obj):
            formula.append([-obj_id], weight=1 if not weights else weights[self.idpool.obj(obj_id)])

        if self.htype == "rc2":
            if not weights or min(weights.values()) == max(weights.values()):
                self.oracle = RC2(
                    formula, solver=self.solver, adapt=self.adapt, exhaust=self.exhaust, minz=self.minz, trim=self.trim
                )
            else:
                self.oracle = RC2Stratified(
                    formula,
                    solver=self.solver,
                    adapt=self.adapt,
            ...
            self.oracle = LBX(formula, solver_name=self.solver, use_cld=self.usecld)
        elif self.htype == "mcsls":
            self.oracle = MCSls(formula, solver_name=self.solver, use_cld=self.usecld)
        else:  # 'sat'
            assert (
                self.solver in SolverNames.minisatgh + SolverNames.lingeling + SolverNames.cadical153
            ), "Hard polarity setting is unsupported by {0}".format(self.solver)

            assert formula.atms == [], (
                "Native AtMostK constraints aren't" "supported by MinisatGH, Lingeling, or CaDiCaL 153"
            )

            # setting up a SAT solver, so that it supports the same interface
            self.oracle = Solver(name=self.solver, bootstrap_with=formula.hard, use_timer=True)
