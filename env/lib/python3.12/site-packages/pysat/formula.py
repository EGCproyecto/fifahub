...
class CNF(object):
    ...
    def to_alien(self, file_pointer, format="opb", comments=None):
        ...
        if format == "opb":
            print("* #variable= {0} #constraint= {1}".format(self.nv, len(self.clauses)), file=file_pointer)
        elif format == "lp":
        ...
        for i, cl in enumerate(self.clauses, 1):
            ...
            if format == "smt":
                ...
                print(
                    "{0} {1} >= {2} {3}".format(
                        "" if format == "opb" else "c{0}:".format(i),
...
class WCNF(object):
    ...
    def from_fp(self, file_pointer, comment_lead=["c"]):
        ...
        self.nv = max(
            map(lambda cl: max(map(abs, cl)), itertools.chain.from_iterable([[[self.nv]], self.hard, self.soft]))
        )
    ...
    def to_alien(self, file_pointer, format="opb", comments=None):
        ...
        if format == "opb":
            print("* #variable= {0} #constraint= {1}".format(self.nv, len(self.hard) + len(hard)), file=file_pointer)
            print(
                "min:",
                " ".join(
                    ["{0}{1} x{2}".format("-" if s[0] > 0 else "+", w, abs(s[0])) for s, w in zip(soft, self.wght)]
                ),
                ";",
                file=file_pointer,
            )
...
class CNFPlus(CNF, object):
    ...
    def __init__(self, from_file=None, from_fp=None, from_string=None, comment_lead=["c"]):
        ...
        super(CNFPlus, self).__init__(
            from_file=from_file, from_fp=from_fp, from_string=from_string, comment_lead=comment_lead
        )
    ...
    def to_fp(self, file_pointer, comments=None):
        ...
        print("p", ftype, self.nv, len(self.clauses) + len(self.atmosts), file=file_pointer)
    ...
    def to_alien(self, file_pointer, format="opb", comments=None):
        ...
        if self.atmosts and format == "smt":
            raise NotImplementedError(
                "SMT-LIB2 does not support PB constraints directly; you may want to use Z3's API instead"
            )
...
class WCNFPlus(WCNF, object):
    ...
    **Note** that every cardinality constraint is assumed to be *hard*,
    ...
    def to_fp(self, file_pointer, comments=None):
        ...
        print("p", ftype, self.nv, len(self.hard) + len(self.soft) + len(self.atms), self.topw, file=file_pointer)
