...
def feature_inclusion_probability(bdd_model: BDDModel, config: Optional[Configuration] = None) -> dict[Any, float]:
    ...
    if config is None:
        ...
        values = {bdd_model.features_variables[f]: selected for f, selected in config.elements.items()}
        for variable, feature in bdd_model.variables_features.items():
