...
def get_prod_dist(
    bdd_model: BDDModel, node: Any, dist: dict[int, list[int]], mark: dict[int, bool], complemented: bool
) -> None:
    ...
    if not bdd_model.is_terminal_node(node):
        ...
        for i in range(removed_nodes + 1):
            for j in range(len(dist[id_high])):
                high_dist[i + j] = high_dist[i + j] + dist[id_high][j] * (math.comb(removed_nodes, i))
        # combine low and high distributions
        combine_distributions(id_node, dist, low_dist, high_dist)


def combine_distributions(id_node: int, dist: dict[int, list[int]], low_dist: list[int], high_dist: list[int]) -> None:
