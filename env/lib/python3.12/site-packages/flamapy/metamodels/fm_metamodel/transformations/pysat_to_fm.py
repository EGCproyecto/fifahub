...
class PysatToFM(ModelToModel):
    ...
    def transform(self) -> FeatureModel:
        ...
        children_features = [Feature(v) for v in self.source_model.variables.keys() if v != root_feature.name]
        ...
        for clause in self.source_model.get_all_clauses():
            if len(clause) != 1:
                node = functools.reduce(
                    lambda left, right: Node(
                        ASTOperation.OR, self._get_node_from_clause_term(left), self._get_node_from_clause_term(right)
                    ),
                    clause,
                )
    ...
    def _identify_root(self) -> Feature:
        ...
        raise FlamaException("Error converting from SAT to FM. ", "There is not candidate for the root feature.")
