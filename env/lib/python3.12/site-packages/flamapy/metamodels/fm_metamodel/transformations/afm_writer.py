...
from flamapy.metamodels.fm_metamodel.models import Feature, FeatureModel, Relation, Attribute
...
class AFMWriter(ModelToText):
    ...
    def read_relation(cls, relation: Relation) -> str:
        ...
        if len(children) == 1:
            ...
            result = "[" + str(relation.card_min) + "," + str(relation.card_max) + "]"
            features = []
            for child in children:
    ...
    def read_attribute(cls, attribute: Attribute) -> str:
        ...
        if len(domain.get_range_list()) > 0:
            ...
            for _range in domain.get_range_list():
                result += "[" + str(_range.min_value) + " to " + str(_range.max_value) + "]"
    ...
    def recursive_constraint_read(self, node: Node) -> str:
        ...
        if node.left and node.right:
            result = self.recursive_constraint_read(node.left) + data + self.recursive_constraint_read(node.right)
