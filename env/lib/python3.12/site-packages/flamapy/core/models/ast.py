...
def simplify_formula(ast: AST) -> AST:
    ...
    if logic_op in (ASTOperation.REQUIRES, ASTOperation.IMPLIES):
        ...
        right = simplify_formula(AST.create_binary_operation(ASTOperation.IMPLIES, right, left)).root
        ...
        left = simplify_formula(AST.create_binary_operation(ASTOperation.AND, left, Node(ASTOperation.NOT, right))).root
        left = simplify_formula(AST.create_binary_operation(ASTOperation.AND, Node(ASTOperation.NOT, left), right)).root
...
def to_cnf(formula: AST) -> AST:
    ...
    if node.data == ASTOperation.AND:
        res = AST.create_binary_operation(ASTOperation.AND, to_cnf(AST(node.left)).root, to_cnf(AST(node.right)).root)
