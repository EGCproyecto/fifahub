...
class Session:
    ...
    def _get_interface(self, app):
        ...
        SESSION_USE_SIGNER = config.get("SESSION_USE_SIGNER", Defaults.SESSION_USE_SIGNER)  # TODO: remove in 1.0
        SESSION_KEY_PREFIX = config.get("SESSION_KEY_PREFIX", Defaults.SESSION_KEY_PREFIX)
        SESSION_ID_LENGTH = config.get("SESSION_ID_LENGTH", Defaults.SESSION_ID_LENGTH)
        SESSION_SERIALIZATION_FORMAT = config.get("SESSION_SERIALIZATION_FORMAT", Defaults.SESSION_SERIALIZATION_FORMAT)
        ...
        SESSION_FILE_THRESHOLD = config.get("SESSION_FILE_THRESHOLD", Defaults.SESSION_FILE_THRESHOLD)
        ...
        SESSION_MONGODB_DB = config.get("SESSION_MONGODB_DB", Defaults.SESSION_MONGODB_DB)
        SESSION_MONGODB_COLLECT = config.get("SESSION_MONGODB_COLLECT", Defaults.SESSION_MONGODB_COLLECT)

        # SQLAlchemy settings
        SESSION_SQLALCHEMY = config.get("SESSION_SQLALCHEMY", Defaults.SESSION_SQLALCHEMY)
        SESSION_SQLALCHEMY_TABLE = config.get("SESSION_SQLALCHEMY_TABLE", Defaults.SESSION_SQLALCHEMY_TABLE)
        SESSION_SQLALCHEMY_SEQUENCE = config.get("SESSION_SQLALCHEMY_SEQUENCE", Defaults.SESSION_SQLALCHEMY_SEQUENCE)
        SESSION_SQLALCHEMY_SCHEMA = config.get("SESSION_SQLALCHEMY_SCHEMA", Defaults.SESSION_SQLALCHEMY_SCHEMA)
        SESSION_SQLALCHEMY_BIND_KEY = config.get("SESSION_SQLALCHEMY_BIND_KEY", Defaults.SESSION_SQLALCHEMY_BIND_KEY)
        SESSION_CLEANUP_N_REQUESTS = config.get("SESSION_CLEANUP_N_REQUESTS", Defaults.SESSION_CLEANUP_N_REQUESTS)

        # DynamoDB settings
        SESSION_DYNAMODB = config.get("SESSION_DYNAMODB", Defaults.SESSION_DYNAMODB)
        SESSION_DYNAMODB_TABLE = config.get("SESSION_DYNAMODB_TABLE", Defaults.SESSION_DYNAMODB_TABLE)
        ...
        if SESSION_TYPE == "redis":
            ...
            session_interface = CacheLibSessionInterface(**common_params, client=SESSION_CACHELIB)
