...
class Parser(object):
    ...
    def build(self, tabmodule=None, outputdir=None, write_tables=False, debug=False, debuglog=None):
    ...
    def _parse_header(self, filename, debuglog):
        ...
        if c == 0:
            logger.info("var IDs")
            id2permid = {i: k for i, k in zip(self.var_ids, self.permuted_var_ids)}
            ...
            self.info2permid = {var: k for k, var in enumerate(self.ordered_vars)}
        elif c == 4:
            logger.info("none")
            raise NotImplementedError
        else:
        ...
        if self.ordered_vars is not None:
            ...
            permid2var = {k: var for k, var in zip(self.permuted_var_ids, self.support_vars)}
            levels = {permid2var[k]: k for k in sorted(self.permuted_var_ids)}
        else:
            levels = {idx: level for level, idx in enumerate(self.permuted_var_ids)}
    ...
    def _parse_body(self, filename):
        ...
        with open(filename, "r") as f:
            ...
            for line in f:
                ...
                assert info in self.info2permid, (info, self.info2permid)
