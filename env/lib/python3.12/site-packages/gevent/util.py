...
def print_run_info(thread_stacks=True, greenlet_stacks=True, limit=_NONE, file=None):
    ...
    .. versionadded:: 1.3b1
    """
    lines = format_run_info(thread_stacks=thread_stacks, greenlet_stacks=greenlet_stacks, limit=limit)
    file = sys.stderr if file is None else file
    for l in lines:
        print(l, file=file)


def format_run_info(thread_stacks=True, greenlet_stacks=True, limit=_NONE, current_thread_ident=None):
    """
...
def _format_greenlet_info(lines, greenlet_stacks, limit):
    ...
    lines.append("*" * 80)
    for tree in sorted(GreenletTree.forest(), key=lambda t: "" if t.is_current_tree else repr(t.greenlet)):
...
class _TreeFormatter(object):
    ...
    def child_head(self, label, right=VERTICAL_AND_RIGHT):
        return " " * self.indent + right + self.HORIZONTAL * self.horiz_width + " " * self.label_space + label
...
class GreenletTree(object):
    ...
    def _render(self, tree):
        ...
        if self.greenlet and tree.details and tree.details["running_stacks"]:
            self.__render_tb(tree, "Running:", self.greenlet.gr_frame, tree.details["running_stack_limit"])
