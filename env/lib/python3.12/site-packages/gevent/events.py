...
class IMemoryUsageUnderThreshold(Interface):
    ...
    mem_usage = Attribute("The current process memory usage, in bytes.")
    max_allowed = Attribute("The maximum allowed memory usage, in bytes.")
    max_memory_usage = Attribute("The memory usage that caused the previous " "IMemoryUsageThresholdExceeded event.")
    memory_info = Attribute("The tuple of memory usage stats return by psutil.")
...
class GeventPatchEvent(object):
    ...
    def __repr__(self):
        return "<%s source=%r target=%r at %x>" % (self.__class__.__name__, self.source, self.target, id(self))
...
class IGeventWillPatchModuleEvent(IGeventWillPatchEvent):
    ...
    module_name = Attribute("The name of the module being patched. " "This is the same as ``target.__name__``.")

    target_item_names = Attribute("The list of item names to patch. " "This can be modified in place with caution.")
...
class _PatchAllMixin(object):
    ...
    def __repr__(self):
        return "<%s %r at %x>" % (self.__class__.__name__, self._patch_all_arguments, id(self))
