        ...
        try:
            ...
            if result is not waiter:
                raise InvalidSwitchError(
                    ...
                    % (getcurrent(), result, waiter, self, watcher)  # pylint:disable=undefined-variable
                )
        finally:
            watcher.stop()
...
def _primitive_wait(watcher, timeout, timeout_exc, hub):
    ...
    timeout = Timeout._start_new_or_dummy(
        timeout, (timeout_exc if timeout_exc is not _NONE or timeout is None else _timeout_error("timed out"))
    )
