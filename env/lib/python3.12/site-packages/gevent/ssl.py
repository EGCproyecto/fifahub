...
class SSLSocket(socket):
    ...
    def _extra_repr(self):
        return " server=%s, cipher=%r" % (self.server_side, self._sslobj.cipher() if self._sslobj is not None else "")
    ...
    def __create_sslobj(self, server_side=False, session=None):
        return self.context._wrap_socket(
            self._sock, server_side, self.server_hostname, owner=self._sock, session=session
        )
...
def get_server_certificate(addr, ssl_version=PROTOCOL_SSLv23, ca_certs=None):
    ...
    with create_connection(addr) as sock:
        with wrap_socket(sock, ssl_version=ssl_version, cert_reqs=cert_reqs, ca_certs=ca_certs) as sslsock:
