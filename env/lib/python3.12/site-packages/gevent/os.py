...
if hasattr(os, "fork"):
    ...
    if hasattr(os, "WNOWAIT") or hasattr(os, "WNOHANG"):
        ...
        def _reap_children(timeout=60):
            ...
            dead = [pid for pid, val in _watched_children.items() if isinstance(val, tuple) and val[2] < oldest_allowed]
            for pid in dead:
            ...
            if pid <= 0:
                ...
                if pid == -1:
                    ...
                    for k, v in _watched_children.items():
                        if isinstance(v, tuple) and (finished_at is None or v[2] < finished_at):
                            pid = k
                            finished_at = v[2]
...
__imports__ = copy_globals(os, globals(), names_to_ignore=__implements__ + __extensions__, dunder_names_to_keep=())
