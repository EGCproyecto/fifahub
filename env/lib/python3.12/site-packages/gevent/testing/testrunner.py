...
class Runner(object):
    ...
    def __call__(self):
        util.log(
            ...
            % (self._worker_count, util._colorize("number", "(concurrency available: %d)" % AVAIL_NWORKERS)),
        )
...
class Discovery(object):
    ...
    class Discovered(object):
        ...
        @staticmethod
        def __makes_no_monkey_patch(contents, _patch_present=re.compile(rb"[^#].*patch_\w*\(")):
    ...
    def discovered(self):
        ...
        discovered = self.Discovered(
            self.package, self.configured_test_options, self.ignore, self.config, self.allow_combine
        )
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Be quiet. Defaults to %(default)s. Also the " "GEVENTTEST_QUIET environment variable.",
    )
    ...
    parser.add_argument(
        "--package", default="gevent.tests", help="Load tests from the given package. Defaults to %(default)s."
    )
    parser.add_argument(
        "--processes",
        "-j",
        ...
        type=int,
        help="Use up to the given number of parallel processes to execute tests. " "Defaults to %(default)s.",
    )
    parser.add_argument(
        "--no-combine", default=True, action="store_false", help="Do not combine tests into process groups."
    )
    parser.add_argument(
        "-u",
        "--use",
    ...
    fail_parser.add_argument(
        "--second-chance", action="store_true", default=False, help="Give failed tests a second chance."
    )
    fail_parser.add_argument(
        "--failfast", "-x", action="store_true", default=False, help="Stop running after the first failure."
    )
