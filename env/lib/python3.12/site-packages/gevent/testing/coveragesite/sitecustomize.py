...
try:
    ...
    if str(e) in ("Can't support concurrency=greenlet with PyTracer, only threads are supported",):
        pass
    elif str(e) in (
        ...
        try:
            ...
            if str(ie) in ("module greenlet._greenlet does not support loading in subinterpreters",):
                ignore = True
        if not ignore:
            raise
