...
class AbstractCallbacks(object):
    ...
    def python_prepare_callback(self, watcher_ptr):
        ...
        if loop is None:  # pragma: no cover
            print("WARNING: gevent: running prepare callbacks from a destroyed handle: ", watcher_ptr)
            return
        loop._run_callbacks()
...
class AbstractLoop(object):
    ...
    def update(self):
        ...
        warnings.warn("'update' is deprecated; use 'update_now'", DeprecationWarning, stacklevel=2)
        self.update_now()

    def __repr__(self):
        return "<%s.%s at 0x%x %s>" % (self.__class__.__module__, self.__class__.__name__, id(self), self._format())
