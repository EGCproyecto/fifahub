...
class TestSemaphoreMultiThread(greentest.TestCase):
    ...
    def _do_test_acquire_in_one_then_another(
        self, release=True, require_thread_acquired_to_finish=False, **thread_acquire_kwargs
    ):
        ...
        t = threading.Thread(
            target=self._makeThreadMain(
                thread_running, thread_acquired, sem, acquired, exc_info, **thread_acquire_kwargs
            )
        )
        t.daemon = True
        t.start()
    ...
    def test_acquire_in_one_then_another_timed(self):
        sem, acquired_in_thread = self._do_test_acquire_in_one_then_another(
            release=False, require_thread_acquired_to_finish=True, timeout=timing.SMALLEST_RELIABLE_DELAY
        )
    ...
    def test_acquire_in_one_wait_greenlet_wait_thread_gives_up(self):
        ...
        thread = threading.Thread(
            target=self._makeThreadMain(
                threading.Event(), threading.Event(), sem, acquired, exc_info, timeout=timing.LARGE_TICK
            )
        )
        thread.daemon = True
        gevent.idle()
