...
class io(_base.IoMixin, watcher):
    ...
    class _multiplexwatcher(object):
        ...
        events = property(
            lambda self: self._events, _base.not_while_active(lambda self, nv: setattr(self, "_events", nv))
        )
        ...
        fd = property(
            lambda self: getattr(self._watcher_ref, "_fd", -1), lambda self, nv: self._watcher_ref._set_fd(nv)
        )
...
class timer(_base.TimerMixin, watcher):
    ...
    def _watcher_ffi_init(self, args):
        ...
        if self._after and self._after < 0.001:
            ...
            warnings.warn(
                "libuv only supports millisecond timer resolution; " "all times less will be set to 1 ms", stacklevel=6
            )
    ...
    def _watcher_ffi_start(self):
        if self._again:
            ...
            try:
                self._watcher_start(
                    self._watcher, self._watcher_callback, int(self._after * 1000), int(self._repeat * 1000)
                )
            except ValueError:
...
class stat(_base.StatMixin, watcher):
    ...
    def _watcher_ffi_start(self):
        ...
        self._watcher_start(self._watcher, self._watcher_callback, self._cpath, int(self._interval * 1000))
