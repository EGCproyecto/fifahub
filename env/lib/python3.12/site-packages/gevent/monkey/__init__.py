...
@_ignores_DoNotPatch
def patch_thread(threading=True, _threading_local=True, Event=True, logging=True, existing_locks=True, _warnings=None):
    """
...
def patch_ssl(_warnings=None, _first_time=True):
    ...
    """
    may_need_warning = _first_time and "ssl" in sys.modules and hasattr(sys.modules["ssl"], "SSLContext")
    ...
    if may_need_warning:
        direct_ref_modules, subclass_modules = _find_module_refs(
            gevent_mod.orig_SSLContext, excluding_names=("ssl", "gevent.ssl", "gevent._ssl3", "gevent._sslgte279")
        )
        if direct_ref_modules or subclass_modules:
            ...
            if direct_ref_modules:
                direct_ref_mod_str = "Modules that had direct imports (NOT patched): %s. " % (
                    ["%s (%s)" % (name, fname) for name, fname in direct_ref_modules]
                )
            if subclass_modules:
                subclass_str = "Subclasses (NOT patched): %s. " % ([str(t) for t in subclass_modules])
            _queue_warning(
                ...
                "See https://github.com/gevent/gevent/issues/1016. " + direct_ref_mod_str + subclass_str,
                _warnings,
            )
...
def _check_repatching(**module_settings):
    ...
    if not first_time and currently_patched != module_settings:
        _queue_warning(
            "Patching more than once will result in the union of all True" " parameters being patched", _warnings
        )
