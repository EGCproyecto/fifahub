...
class local(object):
    ...
    def __getattribute__(self, name):  # pylint:disable=too-many-return-statements
        ...
        raise AttributeError("%r object has no attribute '%s'" % (self._local_type.__name__, name))

    def __setattr__(self, name, value):
        if name == "__dict__":
            raise AttributeError("%r object attribute '__dict__' is read-only" % type(self))
...
if local.__module__ == "gevent.local":
    ...
    if local.__module__ != "gevent._gevent_clocal":  # pylint:disable=else-if-used
        raise AssertionError(
            "Module names changed (local: %r; __name__: %r); revisit this code" % (local.__module__, __name__)
        )
