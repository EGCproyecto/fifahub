...
class Facet(Generic[_R]):
    ...
    def __init__(self, metric: Optional[Agg[_R]] = None, metric_sort: str = "desc", **kwargs: Any):
        self.filter_values = ()
        self._params = kwargs
        self._metric = metric
    ...
    def get_values(self, data: "BucketData[_R]", filter_values: List[FilterValueType]) -> List[Tuple[Any, int, bool]]:
        """
...
class RangeFacet(Facet[_R]):
    ...
    def _range_to_dict(self, range: Tuple[Any, Tuple[Optional[int], Optional[int]]]) -> Dict[str, Any]:
        key, _range = range
...
class FacetedSearchBase(Generic[_R]):
    ...
    def add_filter(self, name: str, filter_values: Union[FilterValueType, List[FilterValueType]]) -> None:
        """
    ...
    def query(self, search: "SearchBase[_R]", query: Union[str, Query]) -> "SearchBase[_R]":
        """
