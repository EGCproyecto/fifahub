...
class AsyncSearch(SearchBase[_R]):
    ...
    async def execute(self, ignore_cache: bool = False) -> Response[_R]:
        ...
        if ignore_cache or not hasattr(self, "_response"):
            ...
            self._response = self._response_class(
                self,
                (await es.search(index=self._index, body=self.to_dict(), **self._params)).body,
            )
    ...
    async def scan(self) -> AsyncIterator[_R]:
        ...
        async for hit in async_scan(es, query=self.to_dict(), index=self._index, **self._params):
    ...
    async def delete(self) -> AttrDict[Any]:
        ...
        return AttrDict(
            cast(
                Dict[str, Any],
                await es.delete_by_query(index=self._index, body=self.to_dict(), **self._params),
            )
        )
    ...
    async def point_in_time(self, keep_alive: str = "1m") -> AsyncIterator[Self]:
        ...
        pit = await es.open_point_in_time(index=self._index or "*", keep_alive=keep_alive)
...
class AsyncMultiSearch(MultiSearchBase[_R]):
    ...
    async def execute(self, ignore_cache: bool = False, raise_on_error: bool = True) -> List[Response[_R]]:
        """
        ...
        if ignore_cache or not hasattr(self, "_response"):
            ...
            responses = await es.msearch(index=self._index, body=self.to_dict(), **self._params)
