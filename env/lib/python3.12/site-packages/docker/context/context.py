...
class Context:
    ...
    def __init__(self, name, orchestrator=None, host=None, endpoints=None, tls=False):
        ...
        if not endpoints:
            ...
            default_endpoint = "docker" if (not orchestrator or orchestrator == "swarm") else orchestrator

            self.endpoints = {default_endpoint: {"Host": get_context_host(host, tls), "SkipTLSVerify": not tls}}
            return
    ...
    def _load_meta(cls, name):
        ...
        try:
            ...
            # unknown format
            raise Exception(f"Detected corrupted meta file for context {name} : {e}") from e
        ...
        for k, v in metadata["Endpoints"].items():
            if k != "docker":
                continue
            metadata["Endpoints"][k]["Host"] = v.get("Host", get_context_host(None, False))
            metadata["Endpoints"][k]["SkipTLSVerify"] = bool(v.get("SkipTLSVerify", True))
