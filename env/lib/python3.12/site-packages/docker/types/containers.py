...
class HostConfig(dict):
    def __init__(
        ...
        if network_mode == "host" and port_bindings:
            raise host_config_incompatible_error("network_mode", "host", "port_bindings")
        ...
        if blkio_weight_device:
            if not isinstance(blkio_weight_device, list):
                raise host_config_type_error("blkio_weight_device", blkio_weight_device, "list")
            if version_lt(version, "1.22"):
        ...
        if device_write_bps:
            if not isinstance(device_write_bps, list):
                raise host_config_type_error("device_write_bps", device_write_bps, "list")
            if version_lt(version, "1.22"):
        ...
        if device_read_iops:
            if not isinstance(device_read_iops, list):
                raise host_config_type_error("device_read_iops", device_read_iops, "list")
            if version_lt(version, "1.22"):
        ...
        if device_write_iops:
            if not isinstance(device_write_iops, list):
                raise host_config_type_error("device_write_iops", device_write_iops, "list")
            if version_lt(version, "1.22"):
        ...
        if device_cgroup_rules is not None:
            ...
            if not isinstance(device_cgroup_rules, list):
                raise host_config_type_error("device_cgroup_rules", device_cgroup_rules, "list")
...
def host_config_type_error(param, param_value, expected):
    return TypeError(f"Invalid type for {param} param: expected {expected} " f"but found {type(param_value)}")
...
def host_config_incompatible_error(param, param_value, incompatible_param):
    return errors.InvalidArgument(f'"{param_value}" {param} is incompatible with {incompatible_param}')
