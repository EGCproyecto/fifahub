...
class ContainerApiMixin:
    ...
    def attach_socket(self, container, params=None, ws=False):
        ...
        return self._get_raw_response_socket(
            self.post(u, None, params=self._attach_params(params), stream=True, headers=headers)
        )

    @utils.check_resource("container")
    def commit(
        self, container, repository=None, tag=None, message=None, author=None, pause=True, changes=None, conf=None
    ):
        """
    ...
    def create_host_config(self, *args, **kwargs):
        ...
        if "version" in kwargs:
            raise TypeError("create_host_config() got an unexpected " "keyword argument 'version'")
    ...
    def logs(
        ...
        if since is not None:
            if isinstance(since, datetime):
                ...
                raise errors.InvalidArgument(
                    "since value should be datetime or positive int/float," f" not {type(since)}"
                )
        ...
        if until is not None:
            ...
            if isinstance(until, datetime):
                ...
                raise errors.InvalidArgument(
                    f"until value should be datetime or positive int/float, " f"not {type(until)}"
                )
    ...
    def stats(self, container, decode=None, stream=True, one_shot=None):
        ...
        if one_shot is not None:
            if utils.version_lt(self._version, "1.41"):
                raise errors.InvalidVersion("one_shot is not supported for API version < 1.41")
            params["one-shot"] = one_shot
        if stream:
            if one_shot:
                raise errors.InvalidArgument("one_shot is only available in conjunction with " "stream=False")
            return self._stream_helper(self._get(url, stream=True, params=params), decode=decode)
        else:
            if decode:
                raise errors.InvalidArgument("decode is only available in conjunction with stream=True")
