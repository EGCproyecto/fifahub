...
class APIClient(
    ...
    __attrs__ = requests.Session.__attrs__ + ["_auth_configs", "_general_configs", "_version", "base_url", "timeout"]
    ...
    def __init__(
        ...
        if tls and not base_url:
            raise TLSParameterError("If using TLS, the base_url argument must be provided.")
        ...
        if base_url.startswith("http+unix://"):
            ...
            if not IS_WINDOWS_PLATFORM:
                raise DockerException("The npipe:// protocol is only supported on Windows")
            try:
                self._custom_adapter = NpipeHTTPAdapter(
                    base_url, timeout, pool_connections=num_pools, max_pool_size=max_pool_size
                )
            except NameError as err:
                raise DockerException("Install pypiwin32 package to enable npipe:// support") from err
            self.mount("http+docker://", self._custom_adapter)
            ...
            try:
                ...
                raise DockerException("Install paramiko package to enable ssh:// support") from err
            ...
            if isinstance(tls, TLSConfig):
                ...
                self._custom_adapter = requests.adapters.HTTPAdapter(pool_connections=num_pools)
        ...
        if not isinstance(self._version, str):
            raise DockerException("Version parameter must be a string or None. " f"Found {type(version).__name__}")
        if utils.version_lt(self._version, MINIMUM_DOCKER_API_VERSION):
            raise InvalidVersion(
                f"API versions below {MINIMUM_DOCKER_API_VERSION} are " f"no longer supported by this library."
            )
    ...
    def _retrieve_server_version(self):
        try:
            ...
            raise DockerException('Invalid response from docker daemon: key "ApiVersion"' " is missing.") from ke
        except Exception as e:
            raise DockerException(f"Error while fetching server API version: {e}") from e
    ...
    def _url(self, pathfmt, *args, **kwargs):
        for arg in args:
            if not isinstance(arg, str):
                raise ValueError(f"Expected a string but found {arg} ({type(arg)}) instead")
