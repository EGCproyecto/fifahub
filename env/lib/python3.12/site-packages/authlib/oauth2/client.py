...
class OAuth2Client:
    ...
    def token_from_fragment(self, authorization_response, state=None):
        ...
        if "error" in token:
            raise self.oauth_error_class(error=token["error"], description=token.get("error_description"))
        self.token = token
        return token
    ...
    def _http_post(self, url, body=None, auth=None, headers=None, **kwargs):
        return self.session.post(url, data=dict(url_decode(body)), headers=headers, auth=auth, **kwargs)
