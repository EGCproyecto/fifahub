...
def create_route(app, engineio_server, engineio_endpoint):  # pragma: no cover
    ...
    endpoint.
    """
    app.add_route(engineio_server.handle_request, engineio_endpoint, methods=["GET", "POST", "OPTIONS"])
    try:
...
def make_response(status, headers, payload, environ):  # pragma: no cover
    ...
    return HTTPResponse(body=payload, content_type=content_type, status=int(status.split()[0]), headers=headers_dict)
