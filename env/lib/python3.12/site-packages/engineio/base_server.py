...
class BaseServer:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.is_asyncio_based() and ("asyncio" not in self._async or not self._async["asyncio"]):  # pragma: no cover
            raise ValueError("The selected async_mode is not asyncio " "compatible")
        if not self.is_asyncio_based() and "asyncio" in self._async and self._async["asyncio"]:  # pragma: no cover
            raise ValueError("The selected async_mode requires asyncio and " "must use the AsyncServer class")
        if transports is not None:
            if isinstance(transports, str):
                transports = [transports]
            transports = [transport for transport in transports if transport in self.valid_transports]
            if not transports:
    ...
    def generate_id(self):
        ...
        id = base64.b64encode(secrets.token_bytes(12) + self.sequence_number.to_bytes(3, "big"))
    ...
    def _upgrades(self, sid, transport):
        ...
        if not self.allow_upgrades or self._get_socket(sid).upgraded or transport == "websocket":
            return []
    ...
    def _ok(self, packets=None, headers=None, jsonp_index=None):
        ...
        if packets is not None:
            ...
            return {
                "status": "200 OK",
                "headers": headers,
                "response": payload.Payload(packets=packets).encode(jsonp_index=jsonp_index).encode("utf-8"),
            }
        else:
            return {"status": "200 OK", "headers": [("Content-Type", "text/plain")], "response": b"OK"}
        ...
        if self.cors_allowed_origins is None:
            ...
            if "wsgi.url_scheme" in environ and "HTTP_HOST" in environ:
                allowed_origins.append(
                    "{scheme}://{host}".format(scheme=environ["wsgi.url_scheme"], host=environ["HTTP_HOST"])
                )
    ...
    def _cors_headers(self, environ):
        ...
        if "HTTP_ACCESS_CONTROL_REQUEST_HEADERS" in environ:
            headers += [("Access-Control-Allow-Headers", environ["HTTP_ACCESS_CONTROL_REQUEST_HEADERS"])]
    ...
    def _log_error_once(self, message, message_key):
        ...
        if message_key not in self.log_message_keys:
            self.logger.error(message + " (further occurrences of this error " "will be logged with level INFO)")
