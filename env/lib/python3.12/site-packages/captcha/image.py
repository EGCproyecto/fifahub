...
class ImageCaptcha:
    ...
    def _draw_character(self, c: str, draw: ImageDraw, color: ColorTuple) -> Image:
        ...
        im = im.rotate(
            ...
            + (secrets.randbits(32) / (2**32)) * (self.character_rotate[1] - self.character_rotate[0]),
            Resampling.BILINEAR,
            expand=True,
        )
        ...
        # warp
        dx2 = (
            w * (secrets.randbits(32) / (2**32)) * (self.character_warp_dx[1] - self.character_warp_dx[0])
            + self.character_warp_dx[0]
        )
        dy2 = (
            h * (secrets.randbits(32) / (2**32)) * (self.character_warp_dy[1] - self.character_warp_dy[0])
    ...
    def generate_image(
        self, chars: str, bg_color: ColorTuple | None = None, fg_color: ColorTuple | None = None
    ) -> Image:
    ...
    def generate(
        self, chars: str, format: str = "png", bg_color: ColorTuple | None = None, fg_color: ColorTuple | None = None
    ) -> BytesIO:
