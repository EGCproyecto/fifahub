...
class FileRequired(DataRequired):
    ...
    def __call__(self, form, field):
        ...
        if not (all(isinstance(x, FileStorage) and x for x in field_data) and field_data):
        ...
        for f in field_data:
            ...
            if (file_size < self.min_size) or (file_size > self.max_size):
                ...
                raise ValidationError(
                    self.message
                    or field.gettext(f"File must be between {self.min_size}" f" and {self.max_size} bytes.")
                )
