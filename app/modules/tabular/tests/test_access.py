...
def _create_tabular_dataset(owner: User, title: str = "Shared Tabular"):
...
def test_other_user_can_view_tabular_dataset(test_client, clean_database):
    ...
    login(test_client, email="other@example.com", password="pwd12345")
    resp = test_client.get(f"/tabular/{dataset_id}")
    assert resp.status_code == 200
    assert b"Shared Tabular" in resp.data
