...
def test_polymorphic_query_returns_subclasses():
    ...
    with app.app_context():
        ...
        md1 = DSMetaData(title="UVL one", description="...", publication_type=PublicationType.BOOK)
        md2 = DSMetaData(title="Tab one", description="...", publication_type=PublicationType.BOOK)
        db.session.add_all([md1, md2])
        db.session.flush()

        # Una instancia de cada subclase
        u = UVLDataset(user_id=user.id, ds_meta_data_id=md1.id)
        t = TabularDataset(user_id=user.id, ds_meta_data_id=md2.id, rows_count=10, schema_json='{"c":3}')
